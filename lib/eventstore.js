...
_.extend(Eventstore.prototype, {
   ...
   * @param events the events that should be saved. The streamRevision for each of the events is assumed to be provided. The events are assumed to be already properly mapped.
   * @param {Function}  callback the function that will be called when this action has finished
   *                             `function(err, addedEvents){}`
   */
  addEvents: function(events, callback) {
    ...
    async.waterfall([
      ...
      function commitEvents(id, callback) {
        // start committing.
        var event,
          uncommittedEvents = [].concat(events);
        ...
        self.store.getNextPositions(uncommittedEvents.length, function(err, positions) {
          ...
          self.store.addEvents(uncommittedEvents, function(err) {
            ...
            const addedEvents = uncommittedEvents;
            callback(null, addedEvents);
          });
        });
      }],
