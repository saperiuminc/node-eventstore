...
function EventstoreWithProjection(opts, store, jobsManager, distributedLock, playbackListStore, playbackListViewStore, projectionStore) {
    ...
    if (options.enableProjection) {
        ...
        this._redisclient = new Redis(options.redisConfig);
        this._redisbclient = new Redis(options.redisConfig);
        this._redissubscriber = new Redis(options.redisConfig);
        ...
        if (self.commit) {
            ...
            self.commit = function(eventstream, callback) {
                ...
                callback = self._enhanceCallback(self, callback, function() {
                    ...
                    if (query) {
                        const topics = self._queryToTopics(query);
                        timing("commit monkeypatch: ", topics, query, new Date());

                        _.each(topics, (topic) => {
                            // async signal
                            const fanoutSignalProducer = self._getFanoutSignalProducer();
                            console.log('signaling topic', topic);
                            fanoutSignalProducer.signal(topic);
                        })
                    }
                });
...
EventstoreWithProjection.prototype.setupNotifyPublish = async function() {
    ...
    //                             const query = self._getQuery(args[0]);
    //                             if (query) {
    //                                 if (self._hasProjections || self._hasSubscriptions) {
    //                                     const topics = self._queryToTopics(query); 
    //                                     timing("commit monkeypatch: ", topics, query, new Date());
    //                                     _.each(topics, (topic) => {
    //                                         // async signal
    //                                         self._fanoutSignalProducer.signal(topic);
    //                                     })
    //                                 }
    //                             }
    //                         });
...
EventstoreWithProjection.prototype._doLeaderElection = async function(projectionGroup) {
    ...
    const fanoutWaitableConsumer = self._getFanoutWaitableConsumer();
    try {
        // TODO: use a better lock key name
        const lockKey = projectionGroup;
        

        let lock = await this._redLock.lock(lockKey, ttlDuration);
        // acquired a lock. do the job
        console.log('acquired lock for key', lockKey);
            ...
            try {
                _.forOwn(this._projections, async (projection, projectionId) => {
                    if (!projectionWaitable[projectionId]) {
                        const topic = self._queryToTopic(projection.query);
                        const waitable = projectionWaitable[projectionId] = await fanoutWaitableConsumer.waitForSignal(topic);

                        waitable.waitRecursive(function(err, done) {
                            const groupedSignalProducer = self._getGroupedSignalProducer();
                            groupedSignalProducer.signal(projectionId);
                            done();
                        });
                    }
                });
                ...
                lock = await lock.extend(ttlDuration);
            } catch (error) {
        ...
        _.forOwn(projectionWaitable, (waitable) => {
            tasks.push(fanoutWaitableConsumer.stopWaitingForSignal(waitable.id));
        });
    ...
    try {
        ...
        if (projection) {
            ...
            for (let i = 0; i < events.length; i++) {
                ...
                const partition = murmurhash.v3(partitionKey) % this.options.playbackEventJobCount;

                let queue = queues[partition];

                if (!queue) {
                    queue = queues[partition] = async.queue(async (event, cb) => {
                        try {
 ...
 * @returns {void} returns void
 */
EventstoreWithProjection.prototype._processCompletedJob = function(jobId, jobData, result) {
    try {
        ...
        if (this.redisPub && jobData._meta && jobData._meta.token) {
            ...
            timing("_processCompletedJob: ", jobData);
        ...
        console.error('_processCompletedJob failed with parameters and error', jobId);
        console.error(JSON.stringify(jobData));
        throw error;
    }
};
...
EventstoreWithProjection.prototype._createRedisClient = function(type) {
    switch (type) {
        case 'client':
            return this._redisclient;
        case 'bclient':
            return this._redisbclient;
        case 'subscriber':
            return this._redissubscriber;
    }
}

EventstoreWithProjection.prototype._getGroupedWaitableConsumer = function() {
    const self = this;
    if (!this._groupedWaitableConsumer) {
        // initialize waitables library
        this._groupedWaitableConsumer = new Waitables.GroupedWaitableConsumer({
            groupId: this.options.projectionGroup,
            createClient: function(type) {
                return self._createRedisClient(type);
            }
        });
    }

    return this._groupedWaitableConsumer;
}


EventstoreWithProjection.prototype._getGroupedSignalProducer = function() {
    const self = this;
    if (!this._groupedSignalProducer) {
        // initialize waitables library
        this._groupedSignalProducer = new Waitables.GroupedSignalProducer({
            createClient: function(type) {
                return self._createRedisClient(type);
            },
            groupId: this.options.projectionGroup
    ...
    return this._groupedSignalProducer;
}

EventstoreWithProjection.prototype._getFanoutSignalProducer = function() {
    const self = this;
...
EventstoreWithProjection.prototype._getFanoutWaitableConsumer = function() {
    const self = this;
...
/**
 * @param {AggregateQuery} query
 * @returns {String} returns the channel
 */
EventstoreWithProjection.prototype._queryToTopic = function(query) {
 ...
 */
EventstoreWithProjection.prototype._queryToTopics = function(query) {
    const topics = [];

    topics.push('all:all:all');
    topics.push(`${query.context}:all:all`);
    topics.push(`${query.context}:${query.aggregate}:all`);
    topics.push(`${query.context}:${query.aggregate}:${query.aggregateId}`);
...
EventstoreWithProjection.prototype._startWaitingForNewEvents = async function(projection) {
    ...
    const groupedWaitableConsumer = this._getGroupedWaitableConsumer();
    console.log('waiting for signal for grouped topic', projection.projectionId);
    const groupedWaitable = await groupedWaitableConsumer.waitForSignal(projection.projectionId);
    groupedWaitable.setTime(30000);
    groupedWaitable.waitRecursive(function(err, done) {
        console.log('got signal from grouped waitable for topic', projection.projectionId);
        self._processJob('job_id', { projection: projection }, null).catch(done).then(data => done(null, data));
    });
}
