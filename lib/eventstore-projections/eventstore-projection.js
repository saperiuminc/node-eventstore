 ...
 * @property {Function} queueJob pass also the lock key
 */
...
function EventstoreWithProjection(options, store) {
    ...
    if (options) {
        ...
        options.group = options.group ? `${options.group}` : 'default';
...
EventstoreWithProjection.prototype._project = async function(projection) {
    ...
    const projectionKey = `projections:${this.options.group}:${projection.projectionId}`;
    if (this.options.distributedLock) {
        const lockKey = projectionKey;
        await this.options.distributedLock.lock(lockKey);
    }
    ...
    if (!projectionExists && jobsManager) {
        ...
        await jobsManager.queueJob(jobParams);
    }

    // unlock
    if (this.options.distributedLock) {
        await this.options.distributedLock.unlock(projectionKey);
    }
 ...
 * @returns {Promise<EventStream>} - returns a stream as promise
 */
EventstoreWithProjection.prototype._getLastEventAsStreamAsync = function(query) {
    return new Promise((resolve, reject) => {
        try {
