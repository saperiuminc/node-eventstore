...
EventstoreWithProjection.prototype._doTaskAssignment = async function(projectionGroup) {
    ...
    //FORREVIEW
    //ONLY ADD PROJECTIONS THAT ARE RUNNING
    _.forOwn(self._projections, async (projection, projectionId) => {
    ...
    // FORREVIEW
    // check member, new _project call should add tasks
    // broadcast to give everyone the task
    // need to tell if receiver is leader then rebalance
    await self._taskGroup.initialize();
    await self._taskGroup.join();

    let currentAssignments = [];
    self._taskGroup.on('rebalance', async (updatedAssignments) => {
        ...
        _.each(updatedAssignments, async function(projectionId) {
            while (currentAssignments.find((item) => item == projectionId)) {
                // FORREVIEW
                // remove tasks if deleteed projection
                ...
                if(projection) {
                    ...
                    try {
                        const lockKey = `projection-locks:${projectionId}`;
                        let lockToken = await self._distributedLock.lock(lockKey, ttlDuration);
                        debug('lock acquired. processing pending messages', projectionId);
                        ...
                        while (continueJob) {
                            try {
                                ...
                                _.forEach(queryArray, function(q) {
                                    ...
                                    waitForAny.push(waitable.waitForSignal(streamId, pollingTimeout));
                                });
                                await Promise.race(waitForAny);
    
                                await self._processProjection(projectionId);
                                if (heartBeat) {
                                ...
                                continueJob = currentAssignments.find((item) => item == projectionId);
                            }
                        };
                    } catch (error) {
        ...
        if(projection) {
            ...
            if (projection.state == 'running') {
                ...
                if (errorFault) {
                    ...
                    //FORREVIEW
                    // add loop to re-process projection
            ...
            console.error('_processJob. storedProjection is missing');
        }
    } catch (error) {
