...
EventstoreWithProjection.prototype._startAllProjections = async function() {
    try {
        ...
        const allProjections =  await this.getProjections();
        ...
        for (let index = 0; index < allProjections.length; index++) {
            ...
            if (!this._projections[projection.projectionId]) {
                await this._startWaitingForNewEvents(projection);
            }
        }
 ...
 */
 EventstoreWithProjection.prototype.getProjections = async function() {
    try {
  ...
  */
 EventstoreWithProjection.prototype.getProjection = async function(projectionId) {
    try {
        return this._projectionStore.getProjection(projectionId);
    } catch (error) {
  ...
  * @returns {Promise<Projection>}
  */
  EventstoreWithProjection.prototype.getProjectionByName = async function(projectionName) {
    try {
        return this._projectionStore.getProjectionByName(projectionName);
    } catch (error) {
        console.error('getProjectionByName failed with parameters and error', projectionId, error);
        throw error;
    }
};
 ...
 * @returns {Promise}
 */
 EventstoreWithProjection.prototype.pauseProjection = async function(projectionId) {
    try {
        return this._projectionStore.setPaused(projectionId);
    } catch (error) {
 ...
 * @returns {Promise}
 */
 EventstoreWithProjection.prototype.runProjection = async function(projectionId) {
    try {
        return this._projectionStore.setRunning(projectionId);
    } catch (error) {
...
EventstoreWithProjection.prototype._processProjection = async function(projection) {
    try {
        ...
        if (projection) {
            if (storedProjection) {
                if (storedProjection.state == 'running') {
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            await this._playbackEvent(event, storedProjection, this.options.eventCallbackTimeout);
                    ...
                    if (errorFault) {
                        await this._projectionStore.setFaulted(storedProjection.projectionId, errorFault);
                    } else {
                        await this._projectionStore.setProcessed(projection.projectionId, Date.now(), eventsCount > 0 ? offset : undefined);
...
EventstoreWithProjection.prototype._startWaitingForNewEvents = async function(projection) {
    ...
    groupedWaitable.setTime(this.options.pollingTimeout);
