...
EventstoreWithProjection.prototype._startAllProjections = async function() {
    try {
        ...
        if (this._jobsManager) {
            ...
            await this._jobsManager.processJobGroup(this, jobGroup, {}, this._processJob, this._processCompletedJob);
        }
    } catch (error) {
...
EventstoreWithProjection.prototype._processJob = async function(jobId, jobData, lastResult) {
    try {
        ...
        console.error('_processJob failed with parameters and error', jobId, error);
        debug(JSON.stringify(jobData));
        debug(JSON.stringify(lastResult));
        throw error;
    }
};
                    ...
                    this._getProjectionState(projection, event).then((projectionState) => {
                        ...
                        eventHandlerFunc(mutableState, event, funcs, (error) => {
                            ...
                            if (error) {
                                ...
                                debug(JSON.stringify(projection));
                                reject(error)
                            } else {
