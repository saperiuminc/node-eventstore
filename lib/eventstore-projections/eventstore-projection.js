...
class EventstoreWithProjection extends Eventstore {
     ...
     */
    async _processProjectionTask(projectionTaskId) {
        // let eventsCount = 0;
        ...
        try {
            ...
            if (projectionTask) {
                ...
                const offset = projectionTask.offset;
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    let jsonOffset = JSON.parse(offset);
                    const jsonLastProcessedOffset = JSON.parse(lastProcessedOffset);

                    if (Array.isArray(jsonOffset) && Array.isArray(jsonLastProcessedOffset) ) { // single concurrent; array offset
                        const newOffsets = [];
                        for (let i = 0; i < jsonOffset.length; i++) {
                            const projectionOffset = JSON.parse(jsonOffset[i]);
                            const eventOffset = JSON.parse(jsonLastProcessedOffset[i]);
                            
                            newOffsets.push(JSON.stringify({
                                c1: projectionOffset.c2,
                                s1: projectionOffset.s2,
                                e1: projectionOffset.e2,
                                c2: eventOffset.c2,
                                s2: eventOffset.s2,
                                e2: eventOffset.e2,
                            }));
                        ...
                        await this._projectionStore.setProcessed(projectionTask.projectionTaskId, Date.now(), newOffsets, !(processedEventsCount > 0));
                    } else if (jsonOffset && jsonLastProcessedOffset) { // multi concurrent; object offset
                        const projectionOffset = JSON.parse(offset) || {};
                        const eventOffset = JSON.parse(lastProcessedOffset) || {};

                        const newOffset = {
                            c1: projectionOffset.c2 || eventOffset.c2,
                            s1: projectionOffset.s2 || eventOffset.s2,
                            e1: projectionOffset.e2 || eventOffset.e2,
                            
                        }

                        if (projectionOffset.c2) {
                            newOffset.c2 = eventOffset.c2;
                            newOffset.s2 = eventOffset.s2;
                            newOffset.e2 = eventOffset.e2;
                        }

                        await this._projectionStore.setProcessed(projectionTask.projectionTaskId, Date.now(), newOffset, !(processedEventsCount > 0));
                    }
