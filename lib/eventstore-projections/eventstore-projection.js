    ...
    ***************************************************************************************************/
        ...
        try {
            ...
            if (shouldUpdateProjectionStore) {
                ...
                if (storedProjection) {
                    ...
                    // TODO: Ryan: Added condition for single partition projection if partitionBy is empty string/nil/undefined
                    if (projectionConfig.partitionBy == 'stream' || _.isFunction(projectionConfig.partitionBy)) {
                        ...
                        let projectionTaskOffset = 0;
                        if (projection && projection.configuration && projection.configuration.fromOffset === 'latest') {
                            projectionTaskOffset = await this._getLatestOffsetAsync();
                        }

                        const newProjectionTask = {
                            projectionTaskId: this._getProjectionTaskId(projection.projectionId),
                            projectionId: projection.projectionId,
                            shard: this.options.shard,
                            offset: projectionTaskOffset
                        };
            ...
            // TODO: Ryan: Filter out only tasks that are of this shard
    ...
    _getProjectionTaskId(projectionId, partition) {
        if (partition) {
            ...
            return `${projectionId}:shard${this.options.shard}`;
        }
