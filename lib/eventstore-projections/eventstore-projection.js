 ...
 * @typedef {Object} PlaybackListStoreConfig
 ...
 * @property {EventstorePlaybackList} EventstorePlaybackList Eventstore playback list library Class
 ...
 * @property {PlaybackListStoreConfig} playbackListStore the field name of the event's name in the payload. Default is "name"
 */
...
EventstoreWithProjection.prototype._registerPlaybackListView = async function(listName, query) {
    try {
        if (!this.options.EventstorePlaybackListView) {
            throw new Error('EventstorePlaybackListView must be provided in the options');
        }

        if (!this.options.playbackListStore) {
            throw new Error('playbackListStore must be provided in the options');
        }

        if (this.options.playbackListStore) {
            if (!this.options.playbackListStore.host) {
                throw new Error('playbackListStore.host must be provided in the options');
            }

            if (!this.options.playbackListStore.port) {
                throw new Error('playbackListStore.port must be provided in the options');
            }

            if (!this.options.playbackListStore.database) {
                throw new Error('playbackListStore.database must be provided in the options');
            }

            if (!this.options.playbackListStore.user) {
                throw new Error('playbackListStore.user must be provided in the options');
            }

            if (!this.options.playbackListStore.password) {
                throw new Error('playbackListStore.password must be provided in the options');
            }
        }

        const EventstorePlaybackListView = this.options.EventstorePlaybackListView;
        const playbackList = new EventstorePlaybackListView({
            host: this.options.playbackListStore.host,
            port: this.options.playbackListStore.port,
            database: this.options.playbackListStore.database,
            user: this.options.playbackListStore.user,
            password: this.options.playbackListStore.password,
            mysql: require('mysql'),
            listName: listName,
            query: query
        });
...
EventstoreWithProjection.prototype._initPlaybackList = async function(projection) {
    ...
    if (projection.playbackList) {
        if (!this.options.EventstorePlaybackList) {
            throw new Error('EventstorePlaybackList must be provided in the options');
        }
        ...
        // const EventstorePlaybackList = require('./eventstore-playback-list');
        const EventstorePlaybackList = this.options.EventstorePlaybackList;
