...
class EventstoreWithProjection extends Eventstore {
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            ...
                            if (!isEventProcessed) {
                                ...
                                await this._playbackEvent(event, projectionConfig, projectionConfig.eventCallbackTimeout || this.options.eventCallbackTimeout, projectionTask.shard, projectionTask.partition);
     ...
     */
    getStateList(listName, state, shard, partition) {
        try {
            // NOTE: ClusteredEventstoreStateList
            const stateList = new EventstoreStateList(this._stateListStore, listName, state, shard, partition);
            return stateList;
        } catch (error) {
    ...
    ***************************************************************************************************/
     ...
     */
    _getPlaybackFunctions(state, shard, partition) {
        const self = this;
        const funcs = {
            ...
            getStateList: function(listName, done) {
                const stateList = self.getStateList(listName, state, shard, partition);
                if (done) {
                    done(null, stateList);
                } else {
     ...
     * @returns {Promise<void>} returns a Promise that resolves to a void
     */
    _playbackEvent(event, projectionConfig, timeout, shard, partition) {
        return new Promise((resolve, reject) => {
                ...
                if (eventHandlerFunc) {
                    if (projectionConfig.outputState === 'true') {
                        this._getProjectionState(projectionConfig, event).then((projectionState) => {
                            ...
                            const result = eventHandlerFunc.call(projectionConfig.playbackInterface, mutableState, event, self._getPlaybackFunctions(mutableState, shard, partition), doneCallback);
                        ...
                        const result = eventHandlerFunc.call(projectionConfig.playbackInterface, null, event, self._getPlaybackFunctions(null, shard, partition), doneCallback);
                        if (self._isPromise(result)) {
                            return result.then(doneCallback).catch(doneCallback);
                        }
                    }
    ...
    _getProjectionStateStreamId(projectionConfig, lastProjectionEvent) {
        ...
        } else if (projectionConfig.partitionBy === 'task') {
            streamId = `${projectionConfig.projectionTaskId}-result`;
