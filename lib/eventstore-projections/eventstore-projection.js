 ...
 * @property {("true"|"false")} filterEvents Filters the events based on the name of the event handlers in the playbackInterface
 */
 ...
 * @constructor
 */
function EventstoreWithProjection(options, store, jobsManager, distributedLock, playbackListStore, playbackListViewStore) {
    const self = this;

    self._processJobQueues = {};
    ...
    if (options) {
        ...
        options.stateContextName = options.stateContextName || 'default';
        options.enableProjection = options.enableProjection === true ? true : false;
        options.commitRedisChannel = `projection-group:${options.projectionGroup}:channel:commited`;
        options.completedJobRedisChannel = `projection-group:${options.projectionGroup}:channel:completed`;
        options.concurrentAggregatesInProjection = options.concurrentAggregatesInProjection || 10;
    }
...
EventstoreWithProjection.prototype.startAllProjections = function(callback) {
    try {
        ...
        const self = this;
        self._setupNotifySubscribe()
        .then(() => {
            // process job groups
            if (self._jobsManager) {
                const jobGroup = self._getProjectionJobGroup();
                self._jobsManager.processJobGroup(self, jobGroup, self._processJob, self._processCompletedJob).then(() => {
                    if (callback) {
                        callback();
                    }
                }).catch((err) => {
...
EventstoreWithProjection.prototype.unsubscribe = function(token) {
    try {
        if (this._tokenChannels[token]) {
            const channel = this._tokenChannels[token];
            const bufferedEventSubscription = this._bufferedEventSubscriptionPool[channel];
            if (bufferedEventSubscription) {
 ...
 * @returns {void} Returns void. Use the callback to the get playbacklist
 */
EventstoreWithProjection.prototype.registerPlaybackListView = function(listName, query, done) {
    this._registerPlaybackListView(listName, query).then(done).catch(done);
};
 ...
 * @returns {void} Returns void. Use the callback to the get playbacklist
 */
EventstoreWithProjection.prototype._registerPlaybackListView = async function(listName, query) {
    try {
                    ...
                    self.commit = function(eventstream, callback) {
                        ...
                        callback = self._enhanceCallback(callback, function() {
                            ...
                            if (query) {
                                ...
                                if(self.redisPub) {
                                    timing("commit monkeypatch: ", self.options.commitRedisChannel, data, new Date());
                                    self.redisPub.publish(self.options.commitRedisChannel, JSON.stringify(data));
                                }
                            }
                        });
        ...
        try {
            ...
            if (self.options.enableProjection === true && self.options.redisConfig && self.redisSub == undefined) {
                ...
                self.redisSub.on('ready', function() {
                    ...
                    self.redisSub.subscribe(self.options.commitRedisChannel, self.options.completedJobRedisChannel);
...
EventstoreWithProjection.prototype._processReceivedRedisMessage = function(redisChannel, strData) {
    ...
    if (redisChannel == self.options.commitRedisChannel) {
    ...
    } else if (redisChannel == self.options.completedJobRedisChannel) {
        if (data.token) {
...
// TODO NOTE: To be moved to Poller
/**
...
EventstoreWithProjection.prototype._processJob = async function(jobId, jobData, lastResult) {
    try {
        // console.log(murmurhash.v3('vehicle_2'));
        ...
        if (projection) {
            ...
            if (projection.filterEvents == 'true') {
                let handlers = [];
                _.forOwn(projection.playbackInterface, function(value, eventHandler) {
                    if (eventHandler !== '$init') {
                        handlers.push(eventHandler);
                    }
                });
                projectionQuery.events = handlers;
            }
            const events = await this._getEventsAsync(projectionQuery, offset, this.options.pollingMaxRevisions);
            ...
            for (let i = 0; i < events.length; i++) {
                ...
                const partition = murmurhash.v3(event.aggregateId) % this.options.concurrentAggregatesInProjection;
                ...
                if (!queue) {
                    queue = queues[partition] = async.queue(async (event, cb) => {
                        try {
                            ...
                            try {
                                await this._saveEventAsync(errorStreamQuery, event.payload);
                            } catch (error) {
...
EventstoreWithProjection.prototype._processCompletedJob = function(jobId, jobData) {
    try {
        ...
        if (this.redisPub && jobData._meta && jobData._meta.token) {
            ...
            this.redisPub.publish(this.options.completedJobRedisChannel, JSON.stringify(data));
        }
        ...
        console.error('_processCompletedJob failed with parameters and error', jobId, jobData);
        throw error;
    }
};
                        ...
                        eventHandlerFunc(mutableState, event, funcs, (error) => {
                            ...
                            if (error) {
                                ...
                                if (!_.isEqual(projectionState.state, mutableState)) {
                                    ...
                                    this._saveProjectionState(projection.projectionId, projectionState.id, mutableState, this.options.stateContextName).then(resolve).catch(reject);
                                } else {
...
EventstoreWithProjection.prototype._saveEventAsync = async function(targetQuery, event) {
    try {
        const stream = await this._getLastEventAsStreamAsync(targetQuery);
 ...
 * @returns {Promise<void>} returns a Promise that resolves to a void
 */
EventstoreWithProjection.prototype._saveProjectionState = async function(projectionId, projectionStateId, newState, stateContextName) {
    const query = {
        aggregateId: projectionStateId,
        aggregate: 'states',
        context: stateContextName
    };
...
EventstoreWithProjection.prototype._getProjectionState = async function(projection, lastProjectionEvent) {
    ...
    const query = {
        ...
        aggregate: 'states',
        context: this.options.stateContextName
    }
...
//     return self._setupNotifySubscribe()
//     .then(() => {
//         return new Promise((resolve, reject) => {
//             try {
//                 let token;
...
//                 timing("_sleepUntilQueryIsNotified subscribe: ", channel, new Date());
//                 token = PubSub.subscribe(channel, function(redisChannel, data) {
//                     timing("_sleepUntilQueryIsNotified: ", redisChannel, data, new Date());
...
EventstoreWithProjection.prototype._getLastEventAsStreamAsync = async function(query) {
    return new Promise((resolve, reject) => {
        try {
            this.getLastEventAsStream(query, (err, stream) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(stream);
...
EventstoreWithProjection.prototype._initPlaybackList = async function(projection) {
    ...
    if (projection.playbackList) {
        ...
        const playbackList = new EventstorePlaybackList({ listName: projection.playbackList.name }, this._playbackListStore);
...
/**
 * Checks the memory of the node application and if memory has reached the threshold, clean up stream buffers
 * @param {String} currentBucket - the current bucket of the stream buffer
 * @param {String} channel - the channel of the stream buffer
 */
// EventstoreWithProjection.prototype._onStreamBufferEventOffered = function(currentBucket, channel) {
//     const self = this;
//     debug(self);
//     if (!self._streamBufferLRULocked) {
//         const heapPercentage = self._getHeapPercentage();
    ...
    if (buckets && buckets.length > 0) {
        ...
        if (channels && channels.length > 0) {
            channels.forEach((channel) => {
                this._streamBufferLRUCleaner.push({ channel });
            });
        }
