...
class EventstoreWithProjection extends Eventstore {
    ...
    async _processProjectionTask(projectionTaskId) {
        ...
        try {
            ...
            if (projectionTask) {
                ...
                // TODO: Feb17: Need to determine if single or multi for proper offset formatting
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            ...
                            errorEvent = event;
                            // TODO: Feb17: Need to determine if single or multi for proper offset formatting
                            errorOffset = this.calculateNewOffset(event, tempOffset);
                            errorOffset = this._serializeProjectionOffset(errorOffset);
        ...
        try {
            ...
            if (projection) {
                ...
                for(const projectionTask of projectionTasks) {
                    const projectionTaskId = projectionTask.projectionTaskId;
                    // TODO: Feb17: Need to determine if single or multi for proper offset formatting
                    let offset = this.formatOffset(0);
    ...
    calculateNewOffset(event, oldOffset) {
      const finalOffset = event.eventSequence;
      return finalOffset;
    }
