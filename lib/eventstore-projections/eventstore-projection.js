...
function EventstoreWithProjection(opts, store, jobsManager, distributedLock, playbackListStore, playbackListViewStore, projectionStore) {
    const self = this;
    ...
    self._streamBufferLRUCleaner = async.queue(function(task, callback) {
        ...
        if (bufferedEventSubscription && bufferedEventSubscription.isActive()) {
            debug('Clearing Stream Buffer only');
            ...
            debug('Closing the Stream Buffer');
    ...
    debug('event store created with options:');
...
EventstoreWithProjection.prototype._startAllProjections = async function() {
    try {
        console.log('_startAllProjections ryan');
        ...
        await this._setupNotifySubscribe();

        if (this._jobsManager) {
            const jobGroup = this._getProjectionJobGroup();

            // process getEvents job group
            this._jobsManager.processJobGroup(this, jobGroup, {}, this._processGetEventsJob, this._processGetEventsCompletedJob);

            // process playbackEvent job group
            // const playbackEventsJobGroupTasks = [];
            _.forOwn(this._projections, (value, key) => {
                for (let i = 0; i < this.options.playbackEventJobCount; i++) {
                    // job group name is {projectionId}-{partition}
                    const jobGroup = `${key}-${i}`;
                    console.log('sub jobGroup', jobGroup);
                    this._jobsManager.processJobGroup(this, jobGroup, {
                        concurrency: 1
                    }, this._processPlaybackEventsJob, this._processPlaybackEventsCompletedJob)
                }
            });

            // await Promise.all(playbackEventsJobGroupTasks);
        }
    } catch (error) {
...
EventstoreWithProjection.prototype.project = function(projection, callback) {
    debug('projection called with params:');
    // just preserving that adrai's public functions are callback based but internally we can just use async/await for elegance
...
EventstoreWithProjection.prototype.subscribe = function(query, revision, onEventCallback, onErrorCallback) {
    ...
    try {
        debug('subscribe called with params: ');
        ...
        // check if subscribe for redis broadcast updates
        self._setupNotifySubscribe().catch((error) => {
            debug('Failed _setupNotifySubscribe on subscribe');
        });
 ...
 * @returns {void} - returns void
 */
EventstoreWithProjection.prototype.setupNotifyPublish = async function() {
    const self = this;
    return new Promise((resolve, reject) => {
        let timeoutHandle = setTimeout(() => {
            console.error('timeout notification for subscribe/projections');
            resolve({});
        }, self.options.eventCallbackTimeout);

        try {
            timing("setupNotifyPublish enableProjection: ", self.options.enableProjection);
            if (self.options.enableProjection === true && self.options.redisConfig && self.redisPub == undefined) {
                if (self.options.redis) {
                    timing("using same redis as jobsmanager");
                    self.redisPub = self.options.redis;
                } else {
                    timing("setupNotifyPublish creating new redis");
                    self.redisPub = new Redis({
                        host: self.options.redisConfig.host,
                        password: self.options.redisConfig.password,
                        port: self.options.redisConfig.port
                    });
                }
                    ...
                    self.commit = function(eventstream, callback) {
                        ...
                        callback = self._enhanceCallback(self, callback, function() {
                            ...
                            if (query) {
                                ...
                                if (self.redisPub) {
                                    timing("commit monkeypatch: ", self._commitRedisChannel, data, new Date());
                                    self.redisPub.publish(self._commitRedisChannel, JSON.stringify(data));
                                }
                            }
                        });
                ...
                timing("setupNotifyPublish finished");
                return resolve({});
            } else {
                timing("setupNotifyPublish not created/already exists: ", self.options.enableProjection);
                if (timeoutHandle) {
                    clearTimeout(timeoutHandle);
                    timeoutHandle = null;
                }
                //NOTE: if no redis OR already exists, just resolve
                return resolve({});
            }
        } catch (error) {
            if (timeoutHandle) {
            ...
            return reject(new Error('error on setupNotifyPublish' + error));
        }
    });
}
 ...
 * @returns {void} - returns void
 */
EventstoreWithProjection.prototype._setupNotifySubscribe = async function() {
    const self = this;
    return new Promise((resolve, reject) => {
        let timeoutHandle = setTimeout(() => {
            console.error('timeout notification for subscribe/projections');
        ...
        try {
            timing("_setupNotifySubscribe enableProjection: ", self.options.enableProjection);
            if (self.options.enableProjection === true && self.options.redisConfig && self.redisSub == undefined) {
                self.redisSub = new Redis({
                    host: self.options.redisConfig.host,
                    password: self.options.redisConfig.password,
                    port: self.options.redisConfig.port
                });
                ...
                self.redisSub.on('ready', function() {
                    timing("_setupNotifySubscribe finished");
                    self.redisSub.subscribe(self._commitRedisChannel, self._completedJobRedisChannel);
                ...
                timing("_setupNotifySubscribe not created/already exists: ", self.options.enableProjection);
                if (timeoutHandle) {
                    clearTimeout(timeoutHandle);
                    timeoutHandle = null;
                }
            ...
            return reject(new Error('error on _setupNotifySubscribe' + error));
        }
    });
}
...
EventstoreWithProjection.prototype._processReceivedRedisMessage = function(redisChannel, strData) {
    ...
    if (redisChannel == self._commitRedisChannel) {
        ...
        timing("_processReceivedRedisMessage commitRedisChannel: ", fullChannel, streamChannel, new Date());
    } else if (redisChannel == self._completedJobRedisChannel) {
        if (data.token) {
            timing("_processReceivedRedisMessage completedJobRedisChannel unsubcribe: ", data.token, new Date());
 ...
 */
EventstoreWithProjection.prototype._processPlaybackEventsJob = async function(jobId, jobData, lastResult) {
    try {
        debug('_processPlaybackEventsJob called');
        const projectionId = jobData.projectionId;
        const handledEvents = jobData.events;
        ...
        const projection = this._projections[projectionId];

        for (let i = 0; i < handledEvents.length; i++) {
            const event = handledEvents[i];
            try {
                ...
                try {
                    await this._saveEventAsync(this._outputEventstore, errorStreamQuery, event.payload);
                } catch (error) {
        ...
        // should not go here. but logging just in case
        console.error('error in _processPlaybackEventsJob with error and params', error, jobId, jobData, lastResult);
    }
};

EventstoreWithProjection.prototype._processPlaybackEventsCompletedJob = function(jobId, jobData) {
    try {
        // TODO: update some store to say that a job is completed for monitoring purposes
        debug(`Playback event job completed with data`);
    } catch (error) {
        console.error('_processPlaybackEventsCompletedJob failed with parameters and error', jobId, jobData);
    }
};
 ...
 * @returns {Promise<void>} returns Promise of type void
 */
EventstoreWithProjection.prototype._processGetEventsJob = async function(jobId, jobData, lastResult) {
    try {
        // TODO: FOR IMPROVEMENT: probably move the creation of playback events job on the _processGetEventsCompletedJob
        // so that the task of a job is to just
        // 1. get the events
        // 2. filter the events
        // 3. partition the events
        // The job will just output it as part of the result. However, might need to check if this is feasible
        // because the result is also saved to redis and storage might get full 


        // jobData is the projection
        const jobProjection = jobData.projection;

        // TO DO: Remove in-memory projection list and use the storedProjection list
        // TO DO: Try to research on how to serialize and deserialize javascript functions
        const projection = this._projections[jobProjection.projectionId];
        const storedProjection = await this._projectionStore.getProjection(jobProjection.projectionId);
        ...
        if (projection) {
            ...
            if (events) {
                debug('got events in getEventsAsync for playback:', jobProjection.projectionId, eventsCount, offset, this.options.pollingMaxRevisions);
            }

            const playbackEventJobHash = {};
            for (let i = 0; i < events.length; i++) {
                const event = events[i];
                // only put to playback event job if event is handled by the projection
                let eventHandlerFunc = this._getEventHandlerFunc(projection, event);

                if (eventHandlerFunc) {
                    const partition = murmurhash.v3(event.aggregateId) % this.options.playbackEventJobCount;
                    if (!playbackEventJobHash[partition]) {
                        playbackEventJobHash[partition] = [];
                    }

                    playbackEventJobHash[partition].push(event);
                }
            }

            const queueJobTasks = [];
            const self = this;
            _.forOwn(playbackEventJobHash, function(value, key) {
                // id is {projectionId}-{partition}-{lastEventSequence}
                // group is  {projectionId}-{partition}
                const job = {
                    id: `${projection.projectionId}-${key}-${offset}`,
                    group: `${projection.projectionId}-${key}`,
                    payload: {
                ...
                // queue this job
                queueJobTasks.push(self._jobsManager.queueJob(job));
            });

            await Promise.all(queueJobTasks);
            ...
            this._projectionStore.setProcessed(projection.projectionId, Date.now(), eventsCount > 0 ? offset : undefined, eventsCount ? 1 : 0);

            // give a clue to the next instance how many events it got
            return {
                gotEventsCount: eventsCount
            }
        }
    } catch (error) {
        console.error('_processGetEventsJob failed with parameters and error', jobId, jobData, lastResult, error);
        throw error;
    }
};
 ...
 * @returns {void} returns void
 */
EventstoreWithProjection.prototype._processGetEventsCompletedJob = function(jobId, jobData, result) {
    try {
        // this is the polling code.
        // notify completion for clearing subscribe PUBSUB
        // no need to notify non-token based job
        if (this.redisPub && jobData._meta && jobData._meta.token) {
            var data = {
                jobId: jobId,
                token: jobData._meta.token
            };
            timing("_processGetEventsCompletedJob: ", jobData);
            this.redisPub.publish(this._completedJobRedisChannel, JSON.stringify(data));
        }
        ...
        if (projection) {
            ...
            let delay = this.options.pollingTimeout;

            // if the number of events that we got is the same number as our max revisions,
            // then it means we still have events to process. so we do not delay
            if (result && result.gotEventsCount == this.options.pollingMaxRevisions) {
                delay = undefined;
                debug('_processGetEventsCompletedJob still has events. do not delay');
            }
            this._queueProjection(projectionKey, projection, delay);
        }
    } catch (error) {
        console.error('_processGetEventsCompletedJob failed with parameters and error', jobId, jobData);
        throw error;
    }
};
                        ...
                        eventHandlerFunc(mutableState, event, funcs, (error) => {
                            ...
                            if (error) {
                                ...
                                if (!_.isEqual(projectionState.state, mutableState)) {
                                    debug('state changed, saving new state');
...
EventstoreWithProjection.prototype._getProjectionState = async function(projection, lastProjectionEvent) {
    ...
    debug('getting projection last state of streamId');
    const lastEvent = await this._getLastEventAsync(this._outputEventstore, query);

    debug('got projection last state of streamId');
...
EventstoreWithProjection.prototype._getProjectionStateStreamId = function(projection, lastProjectionEvent) {
    ...
    debug('_getProjectionStateStreamId streamId');
...
EventstoreWithProjection.prototype._project = async function(projection) {
    try {
        ...
        if (this._distributedLock) {
            ...
            lockToken = await this._distributedLock.lock(lockKey);
        }
        ...
        // initialize the state list. make sure that it is also inside the lock
        // await this._initStateList(projection);

        // unlock
        if (this._distributedLock && lockToken) {
...
EventstoreWithProjection.prototype._queueProjection = async function(projectionKey, projection, delay) {
    ...
    const jobsManager = this._jobsManager;
    const jobId = projectionKey;

    const jobGroup = this._getProjectionJobGroup();
    if (jobsManager) {
        const token = await this._waitUntilNotifyProject(jobId, projection.query, delay);
...
EventstoreWithProjection.prototype._waitUntilNotifyProject = async function(jobId, query, delay) {
    try {
        ...
        if (delay > 0) {
            ...
            timing("_waitUntilNotifyProject subscribe: ", channel, new Date());
            const token = PubSub.subscribe(channel, function(redisChannel, data) {
                timing("_waitUntilNotifyProject received: ", redisChannel, data, new Date());
                timing("_waitUntilNotifyProject jobId token: ", jobId, token);

                // Loop through known jobs, then  promote
                const jobGroup = self._getProjectionJobGroup();
                PubSub.unsubscribe(token);
                timing("_waitUntilNotifyProject promoteJob: ", jobId, jobGroup, new Date());
                self._jobsManager.promoteJob(jobId, jobGroup);
            });
            return token;
        } else {
            return;
...
EventstoreWithProjection.prototype._addEventToStream = async function(stream, event) {
    debug('_addEventToStream called');
    return new Promise((resolve, reject) => {
        try {
