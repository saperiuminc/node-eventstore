...
EventstoreWithProjection.prototype._processReceivedRedisMessage = function(redisChannel, strData) {
    ...
    timing("_processReceivedRedisMessage: ", redisChannel, data, new Date());
    ...
    if (redisChannel == self.options.notifyCommitRedisChannel) {
        ...
        timing("_processReceivedRedisMessage notifyCommitRedisChannel: ", fullChannel, streamChannel);
    } else if (redisChannel == self.options.notifyCompletedJobRedisChannel) {
        if (self._jobs && data.token) {
            const job = self._jobs[data.token];
            timing("_processReceivedRedisMessage notifyCompletedJobRedisChannel unsubcribe: ", job);
            if (job) {
                PubSub.unsubscribe(data.token);
                delete self._jobs[data.token];
            }
        }
    }
}
    ...
    try {
        ...
        if (delay > 0) {
            ...
            const token = PubSub.subscribe(channel, function(redisChannel, data) {
                ...
                timing("_waitUntilNotifyProject self._jobs: ", self._jobs);
