...
class EventstoreWithProjection extends Eventstore {
    constructor(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore) {
        ...
        // const eventstoreUseEventPublisher = super.useEventPublisher;
        ...
        // eventstoreUseEventPublisher.apply(this, [function(event, done) {
        //     if (self._useEventPublisherCallback) {
        //         self._useEventPublisherCallback(event, function() {
        //             // TODO: this also assumes that es.defineEventMappings have aggregateId or the event has aggregateId
        //             self._sendSignal({
        ...
        // super.useEventPublisher = function(callback) {
    ...
    init(callback) {
        // NOTE: if enableProjection is true AND if no publisher OR useEventPublisher is not called before init, then
        // call useEventPublisher to wrap the signaling code 
        if (!this.publisher && this.options.enableProjection) {
            this.useEventPublisher(function () {});
        }

        super.init(callback);
    }

    useEventPublisher(callback) {
        const self = this;
        this._useEventPublisherCallback = callback;
        super.useEventPublisher(function (event, done) {
            if (self._useEventPublisherCallback) {
                self._useEventPublisherCallback(event, function() {
                    // TODO: this also assumes that es.defineEventMappings have aggregateId or the event has aggregateId
