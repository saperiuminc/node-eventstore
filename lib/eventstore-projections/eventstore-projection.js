...
EventstoreWithProjection.prototype._doLeaderElection = async function(projectionGroup) {
    ...
    try {
            ...
            try {
                _.forOwn(this._projections, async (projection, projectionId) => {
                    if (!projectionWaitable[projectionId]) {
                        let queryArray = [];
                        projectionWaitable[projectionId] = {};
                        ...
                        _.forEach(queryArray, async function(q) {
                            ...
                            const waitable = projectionWaitable[projectionId][channel] = await self._distributedSignal.waitForSignal(channel, null);
        ...
        // cleanup
        const tasks = [];
        _.forOwn(projectionWaitable, (waitables) => {
            _.forOwn(waitables, (waitable) => {
        ...
        try {
            ...
            const funcs = {
                ...
                getStateList: function(listName, done) {
                    self.getStateList(listName, done);
                }
            };
            ...
            if (eventHandlerFunc) {
                if (projection.outputState === 'true') {
                    this._getProjectionState(projection, event).then((projectionState) => {
                        ...
                        const mutableState = _.cloneDeep(projectionState.payload);

                        // TODO: use playbackInterface as the caller of eventHandlerFunc
                        // so that we can use the this variable to call methods of playbackInterface
                        eventHandlerFunc(mutableState, event, funcs, (error) => {
                            ...
                            if (error) {
                                ...
                                if (!_.isEqual(projectionState.payload, mutableState)) {
                                    debug('state changed, saving new state', projectionState.payload, mutableState);
                                    // add event metadata to the state
                                    mutableState._meta = {
                                        fromEvent: event
                                    };
                                    projectionState.payload = mutableState;
                                    this._saveProjectionState(projectionState).then(resolve).catch(reject);
                                } else {
...
/**
* @param {ProjectionState} projectionState the projectionState
* @returns {Promise<void>} returns a Promise that resolves to a void
*/
EventstoreWithProjection.prototype._saveProjectionState = async function(projectionState) {
    const query = {
        aggregateId: projectionState.aggregateId,
        aggregate: projectionState.aggregate,
        context: projectionState.context
    };

    await this._saveEventAsync(this._outputEventstore, query, projectionState);
};
...
EventstoreWithProjection.prototype._getProjectionState = async function(projection, lastProjectionEvent) {
    ...
    const aggregate = projection.resultStreamName || 'states';
    const context = this._outputEventstore._context;
    const query = {
    ...
    if (lastEvent) {
        ...
        projectionState = lastEvent.payload;
    } else {
        projectionState = {
            aggregateId: streamId,
            aggregate: aggregate,
        ...
        projectionState[this._outputEventstore.options.eventNameFieldName] = `${aggregate}_state_changed`;
        ...
        if (typeof projection.playbackInterface.$init === 'function') {
            try {
                ...
                if (initState) {
                    projectionState.payload = initState;
                }
            } catch (error) {
...
EventstoreWithProjection.prototype._getProjectionStateStreamId = function(projection, lastProjectionEvent) {
    ...
    let streamId;
    if (projection.resultStreamName) {
        streamId = projection.resultStreamName;
        if (projection.partitionBy === 'stream') {
            let streamPartitionedStreamId = `${projection.resultStreamName}`;
            // format: <resultStreamName>-<aggregateId>
            ...
            try {
                ...
                if (partitionId) {
                    ...
                    streamId = `${projection.resultStreamName}-${partitionId}-result`;
                }
        ...
        streamId = `${projection.projectionId}-result`;
        if (projection.partitionBy === 'stream') {
    ...
    if (projection.stateList) {
        ...
        for (let index = 0; index < stateLists.length; index++) {
            ...
            const stateList = new EventStoreStateList({
                ...
                user: this.options.listStore.user,
                password: this.options.listStore.password,
                listName: stateListConfig.name,
                fields: stateListConfig.fields,
                secondaryKeys: stateListConfig.secondaryKeys
            });
            await stateList.init();

            this._stateLists[stateListConfig.name] = stateList;
        }
    }
};
