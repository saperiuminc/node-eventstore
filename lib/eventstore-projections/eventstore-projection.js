...
EventstoreWithProjection.prototype.startAllProjections = function(callback) {
    try {
        ...
        if (this.options.jobsManager) {
            ...
            this.options.jobsManager.processJobGroup(jobGroup, this._processJob, this._processCompletedJob).then(() => {
                if (callback) {
                    callback();
                }
            }).catch((err) => {
...
/**
 * @param {import('./jobs-manager').Job} job the job to process
 * @param {import('./jobs-manager').JobDoneCallback} done callback to notify that the job is done
 * @returns {Promise<void>} returns Promise of type void
 */
EventstoreWithProjection.prototype._processJob = async function(job, done) {
    try {
        const projection = job.payload;

        let offset = job.userData ? parseInt(job.userData.lastOffset) : 0;
        ...
        console.error('_processJob failed with parameters and error', job, error);
        done(error);
    }
};
 ...
 * @returns {void} returns void
 */
EventstoreWithProjection.prototype._processCompletedJob = function(job) {
    // this is the polling code.
    // queue the same job again to poll with a delay
    this.options.jobsManager.queueJob(job, {
        delay: this.options.pollingTimeout
    });
};
