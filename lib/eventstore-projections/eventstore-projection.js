...
class EventstoreWithProjection extends Eventstore {
    ...
    async _project(projectionConfig, shouldUpdateProjectionStore) {
        ...
        try {
            ...
            let lockToken;
            const lockKey = `${projectionKey}-shard`;
            try {
            ...
            if (shouldUpdateProjectionStore) {
                ...
                if (storedProjection) {
                    ...
                    debugC('ES: UPDATED PROJECTION', projectionConfig.projectionId);
                } else {
                    await this._projectionStore.createProjection(projection);
                    debugC('ES: CREATED PROJECTION', projectionConfig.projectionId);
                }

                // abtract method
                projectionTasks = await this.createProjectionTasks(projection.projectionId);
            } else {
                debugC('ES: shouldUpdateProjectionStore false', projectionConfig.projectionId);
            }
            ...
            for (const projectionTask of projectionTasks) {
                ...
                debugC('ES: SETTING TASKS IN MEMORY', projectionConfig.projectionId);
            }
    ...
    runProjection(projectionId, forced, callback) {
        this._runProjection(projectionId, forced).then(callback).catch(callback);
    };
    ...
    async _startAllProjections() {
        try {
            ...
            const projectionTasks = await this._getProjectionTasks();
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            ...
                            errorFault = error;
                            errorEvent = event;
                            const errorOffsetNum = this.calculateNewOffset(event, tempOffset);
                            errorOffset = this._serializeProjectionOffset(errorOffsetNum);
                    ...
                    debug('Set processed!!', projectionTask.projectionTaskId, Date.now(), eventsCount > 0 ? this._serializeProjectionOffset(offset) : undefined, !(eventsCount > 0))
    ...
    async _resetProjection(projectionId) {
        try {
            ...
            if (projection) {
                ...
                for(const projectionTask of projectionTasks) {
                    const projectionTaskId = projectionTask.projectionTaskId;
                    let offset = this.resetOffset(projectionTask.partition);
                    await this._projectionStore.setOffset(projectionTaskId, this._serializeProjectionOffset(offset));
                }
            }
    ...
    */
    resetOffset(partition) {
      throw new Error('resetOffset must be implemented');
    }
    /**
     * 
    ...
    formatOffset(rawOffset) {
      throw new Error('formatOffset must be implemented');
    }
    /**
     * 
     * @param {Object} event 
    ...
    calculateNewOffset(event, oldOffset) {
      throw new Error('calculateNewOffset must be implemented');
    }
     ...
     */
     sendSignal(query, done) {
        ...
        // TODO: Feb17: Need to get this somewhere. For each shard?
