        ...
        try {
            ...
            if (projectionTask) {
                if (signaledEvents && signaledEvents.length > 0) {
                    // console.log(overrideEvent);
                    // console.log('CHECK OVERRIDE EVENT');

                    // const offsetToSerialize = {
                    //     eventId: overrideEvent.eventId,
                    //     streamRevision: overrideEvent.streamRevision,
                    //     commitStamp: overrideEvent.commitStamp
                    // };

                    // TODO: sort the signaledEvents array and get the earliest event
                    // TODO: compare the earliest event to the projectionTask.offset

                    // if (!_.isEmpty(signalResult.messages)) {
                    //     const events = signalResult.messages;
                    //     events.sort((event1, event2) => {
                    //         return self.store.getOffsetManager().compareOffset(event1.eventSequence, event2.eventSequence);
                    //     });

                    //     const earliestEvent = events[0];
                    //     if (projectionTask.offset != null && self.store.getOffsetManager().compareOffset(earliestEvent.eventSequence, projectionTask.offset) < 0) {
                    ...
                    // projectionTask.offset = self.store.getOffsetManager().serializeOffset(offsetToSerialize);
                }
                const projectionConfig = projectionTask.projection.configuration;
                let offset = projectionTask && projectionTask.offset ? projectionTask.offset : undefined;
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    debugC('ES: PROJECTION TASK GET EVENTS AT OFFSET', offset, projectionTaskId, projectionTask.partition);
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            // Check for gap here
                            const projectionStreamVersion = await this._projectionStore.getProjectionStreamVersion(projectionTask.projectionId, event.context, event.aggregate, event.aggregateId);
                            // console.log('===========');
                            ...
                            if (event.streamRevision > projectionStreamVersion || !projectionStreamVersion) {
                    ...
                    debugC('Set processed!!', projectionTask.projectionTaskId, Date.now(), eventsCount > 0 ? offset : undefined, !(eventsCount > 0))
                    await this._projectionStore.setProcessed(projectionTask.projectionTaskId, projectionTask.projectionId, Date.now(), eventsCount > 0 ? offset : undefined, !(eventsCount > 0), lastEventByStreams);
