                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    try {
                        ...
                        if (projectionConfig.concurrentEventNames &&
                            ...
                            projectionConfig.concurrencyCount > 0 &&
                            projectionConfig.concurrentEventNames.includes(eventName)) {
                            if (!batchQueue) {
                                batchQueue = async.queue(function(event, done) {
                                    try {
                                        self._playbackEvent(event, projectionConfig, projectionConfig.eventCallbackTimeout || self.options.eventCallbackTimeout)
                                            .then(() => done())
                                            .catch((err) => done(err));
                                    } catch (error) {
                                        done(error);
                                    }
                                }, projectionConfig.concurrencyCount);
                            }
                            ...
                            if (endTheBatch) {
                                ...
                                debugBatch('ending the batch with length', batchQueue.length());
                                const waitForQueueToDrain = function() {
                                    return new Promise((resolve) => {
                                        batchQueue.drain = resolve;
                                    });
                                }
