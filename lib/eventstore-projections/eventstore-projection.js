 ...
 */
 EventstoreWithProjection.prototype.runProjection = function(projectionId, done) {
    try {
        this._projectionStore.setState(projectionId, 'running').then((data) => done(null, data)).catch(done);
    } catch (error) {
    ...
    try {
        ...
        if (projection) {
            if (projection.state == 'running') {
                ...
                if (errorFault) {
                    await this._projectionStore.setStateWithError(projection.projectionId, 'faulted', offset, errorFault, errorEvent);
