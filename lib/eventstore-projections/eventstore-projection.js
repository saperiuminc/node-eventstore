...
function EventstoreWithProjection(options, store) {
    ...
    self._streamBufferLRUCleaner = async.queue(function(task, callback) {
        ...
        self._streamBuffers[channel].cleanUp();
        callback();
    });
    ...
    try {
        ...
        if (!self._streamBuffers[channel]) {
            ...
            const options = {
                ...
                onOfferEvent: self._onStreamBufferEventOffered,
                onCleanUp: (bucket, channel) => {
        ...
        if (!self._eventPollingSubscriptionPool[channel]) {
            ...
            console.log('CREATING NEW EventPollingSubscription FOR: ', channel);
            self._eventPollingSubscriptionPool[channel] = new EventPollingSubscription(pollerOptions);
        } else {
            console.log('FOUND EXISTING EventPollingSubscription FOR: ', channel);
        }
        const eventPollingSubscription = self._eventPollingSubscriptionPool[channel];

        eventPollingSubscription.subscribe(token, revision, onEventCallback);
...
EventstoreWithProjection.prototype._onStreamBufferEventOffered = function(currentBucket, channel) {
    if (!this._streamBufferLRULocked) {
        const heapPercentage = this._getHeapPercentage();
        if (heapPercentage >= STREAM_BUFFER_MEMORY_THRESHOLD) {
            this._cleanOldestStreamBufferBucket();
        }
    }

    const newBucket = this._getCurrentStreamBufferBucket();
    if (this._streamBufferBuckets[currentBucket]) { 
        delete this._streamBufferBuckets[currentBucket][channel];
    }
    
    if (!this._streamBufferBuckets[newBucket]) {
        this._streamBufferBuckets[newBucket] = {};
    }

    this._streamBufferBuckets[newBucket][channel] = new Date().getTime();
    this._streamBuffers[channel].bucket = newBucket;
}
