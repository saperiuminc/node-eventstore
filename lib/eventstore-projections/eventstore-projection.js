...
const Waitable = require('./waitables');
...
EventstoreWithProjection.prototype._doTaskAssignment = async function(projectionGroup) {
    ...
    const waitable = Waitable.createWaitable({
            ...
            while (currentAssignments.find((item) => item == projectionId) && currentRebalanceId === rebalanceId) {
                ...
                if (projection) {
                    ...
                    try {
                        ...
                        pubsub = require('./pubsub/index').createPubSubClient({
                            createRedisClient: self.options.redisCreateClient
                        });

                        
                        // TODO: probably make this an object so that it is easier to read and the logic is cohesive there
                        const subscriptionSubscibers = [];
                        let gotMessageOnNoSubscribersCounter = 0;

                        const waitForSubscriptionMessages = async function(callback) {
                            if (subscriptionTokens.length == 0) {
                                ...
                                for (const q of queryArray) {
                                    ...
                                    const subToken = await pubsub.subscribe(streamId, function(err, message) {
                                        if (subscriptionSubscibers.length > 0) {
                                            while(subscriptionSubscibers.length > 0) {
                                                const sub = subscriptionSubscibers.pop();
                                                sub('subscription');
                                            }
                                        } else {
                                            gotMessageOnNoSubscribersCounter++;
                                        }
                                    });
        
                                    subscriptionTokens.push(subToken);
                                }
                            }

                            if (gotMessageOnNoSubscribersCounter > 0) {
                                gotMessageOnNoSubscribersCounter = 0;
                                callback('pending');
                            } else {
                                subscriptionSubscibers.push(callback);
                            }
                        }
                        ...
                        while (continueJob) {
                            try {
                                ...
                                if (tryContinueProcessProjection == false) {
                                    const waitForAnySubscriptions = new Promise((resolve) => {
                                        waitForSubscriptionMessages(resolve);
                                    });

                                    const waitTimeout = self._sleep(self.options.pollingTimeout);

                                    reason = await Promise.race([waitForAnySubscriptions, waitTimeout]);
                                }
                        ...
                        for (const subToken of subscriptionTokens) {
                            await pubsub.unsubscribe(subToken);
                        }
                    }
                } else {
