...
function EventstoreWithProjection(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore) {
    ...
    if (options.enableProjection) {
        ...
        // TODO: this means that you can only do signaling when enableProjection is set to true
        const useEventPublisher = this.useEventPublisher;
        
        useEventPublisher.apply(this, [function(event, done) {
        ...
        this.useEventPublisher = function(callback) {
            self._useEventPublisherCallback = callback;
        }

        self._distributedQueues = {};
    }

    this._requiredEventMapping = {
        context: 'context',
        aggregate: 'aggregate',
        aggregateId: 'aggregateId'
    }

    this.defineEventMappings(this._requiredEventMapping);
 ...
 * @returns 
 */
 EventstoreWithProjection.prototype.defineEventMappings = function(mappings) {
    ...
    if (!mappings || !_.isObject(mappings)) {
      ...
      const combinedMapping = _.defaults(mappings, this._requiredEventMapping);
  
      this.eventMappings = combinedMapping;
 ...
 */
 EventstoreWithProjection.prototype._sendSignal = function(query, done) {
    const self = this;
    this.store.getPartition(query.aggregateId, function(err, partition) {
...
EventstoreWithProjection.prototype._deleteProjection = async function(projectionId) {
    try {
        ...
        const queue = this._distributedQueues[projectionId];
        if (queue) {
 ...
 * @returns {void} Returns void. Use the callback to the get playbacklist
 */
EventstoreWithProjection.prototype._registerPlaybackListView = async function(listName, listQuery, totalCountQuery, alias, paginationPrimaryKeys, defaultPaginationPrimaryKeySortDirection, forceIndexHints, ignoreIndexHints) {
    try {
...
EventstoreWithProjection.prototype.rebalance = async function(updatedAssignments, rebalanceId) {
    const self = this;
    ...
    _.each(updatedAssignments, async function(projectionId) {
        while (self._currentAssignments.find((item) => item == projectionId) && self._currentRebalanceId === rebalanceId) {
            try {
                ...
                if (projection) {
                    ...
                    try {
                        const lockKey = `projection-locks:${self.options.projectionGroup}:${projectionId}`;
                        ...
                        for (const q of queryArray) {
                            // TODO: move getting of the shard and partition out of the projection config
                            const streamId = self._getChannel(q, projectionConfig.shard, projectionConfig.partition);
                        ...
                        while (continueJob) {
                            try {
                                ...
                                continueJob = (self._currentAssignments.find((item) => item == projectionId) && self._currentRebalanceId === rebalanceId);
                            }
                        };
                    } catch (error) {
                    ...
                    const foundAssignment = self._currentAssignments.find((item) => item == projectionId);
                    if (foundAssignment) {
 ...
 */
EventstoreWithProjection.prototype._getChannel = function(query, shard, partition) {
...
EventstoreWithProjection.prototype._processProjection = async function(projectionId) {
    ...
    try {
        ...
        if (projection) {
            ...
            if (projection.state == 'running' || (projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                let projectionQuery = _.clone(projectionConfig.query);


                // add partition to query
                if(!_.isNil(projectionConfig.shard) && !_.isNil(projectionConfig.partition)) {
                    if(_.isArray(projectionQuery)) {
                        for(let query of projectionQuery) {
                            query.partition = projectionConfig.partition;
                            query.shard = projectionConfig.shard;
                        }
                    } else {
                        projectionQuery.partition = projectionConfig.partition;
                        projectionQuery.shard = projectionConfig.shard;
                    }
                }
                ...
                if (eventsCount) {
                    debugPP('PROJECTION', projection);
                    debugPP('got events in getEventsAsync for playback:', events, events.length, projectionConfig.shard, projectionConfig.partition, projectionQuery);
                }
                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    try {
                        ...
                        if (projectionConfig.concurrentEventNames &&
                            projectionConfig.concurrentEventNames.length > 0 &&
                            projectionConfig.concurrentEventNames.includes(eventName)) {
                            if (!batchQueue) {
                                batchQueue = new BullQueue(projectionId, {
                            ...
                            let endTheBatch = false;
                            const nextEvent = i + 1 < events.length ? events[i + 1] : null;
                            if (nextEvent) {
                                const nextEventName = nextEvent.payload[this.options.eventNameFieldName];
                                if (nextEventName === eventName) {
                                    debugBatch('event the same adding to batch', eventName, events.length)
                                } else {
                                    debugBatch('event different ending the batch', nextEventName, eventName, events.length);
                                    endTheBatch = true;
                                }
                            } else {
                                debugBatch('no next event. ending the batch', eventName, events.length);
                                endTheBatch = true;
                            }
                            
                            if (endTheBatch) {
                                const startTime = Date.now();
                                debugBatch('ending the batch with length', await batchQueue.count());
                                const waitForQueueToDrain = function() {
                                    return new Promise((resolve) => {
                                        batchQueue.on('global:drained', () => {
                                            resolve();
                                        });
                                    });
                                }
                                const endTime = Date.now();
                                debugBatch('batch ended', endTime - startTime);
...
EventstoreWithProjection.prototype._getProjectionEvents = async function(eventstore, query, offset, limit) {
    ...
    // NOTE: query can be an array
    const channel = self._getChannel(query, query.shard, query.partition);
 ...
 */
EventstoreWithProjection.prototype._queryToChannels = function(query, shard, partition) {
    const channels = [];
...
EventstoreWithProjection.prototype._project = async function(projectionConfig, shouldUpdateProjectionStore) {
    try {
        ...
        if (shouldUpdateProjectionStore) {
            ...
            if (storedProjection) {
                ...
                if (projection && projection.configuration && projection.configuration.fromOffset === 'latest') {
                    // TODO: review how we get the partition. currently the config is getting mutated to pass the partition
                    projection.offset = await this._getLatestOffsetAsync(projectionConfig.partition);
                }
        ...
        // listen to distributed jobs
        const queue = this._distributedQueues[projection.projectionId] = new BullQueue(projection.projectionId, {
            createClient: this.options.redisCreateClient
        });

        queue.process(async (job) => {
            try {
                await this._playbackEvent(job.data, projectionConfig, this.options.eventCallbackTimeout);
            } catch (error) {
                console.error('error in playing back event in distributed queue', error);
                throw error;
            }
        })
 ...
 * @returns {Promise<void>} returns a Promise of type void
 */
EventstoreWithProjection.prototype._getLatestOffsetAsync = async function(partition) {
    return new Promise((resolve, reject) => {
