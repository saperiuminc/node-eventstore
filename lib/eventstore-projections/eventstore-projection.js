...
function EventstoreWithProjection(options, store) {
    ...
    this._subscribeChannel = 'EVENTSTORE_SBUSCRIBE_CHANNEL';
    this._subscriptions = {};
    this._projections = {};
    this._playbackLists = {};
 ...
 * @returns {void} - returns void
 */
EventstoreWithProjection.prototype._setupPubSub = function(options) {
    const self = this;
    ...
    self.redisSub.on('ready', function() {
        self.redisSub.subscribe(self._subscribeChannel);
    });

    this.redisSub.on('message', function(channel, message) {
        let msg = JSON.parse(message);
        PubSub.publish(msg.channel, msg.revision);
    });

    function enhanceCallback(callback, preCallback) {
    ...
    if (self.commit) {
        ...
        self.commit = function (eventstream, callback) {
            ...
            callback = enhanceCallback(callback, function() {
                const channel = self._getChannel(args[0]);
                if (channel) {
                    var payload = {
                        channel: channel,
                        revision: args[0].lastRevision
                    };
                    self.redisPub.publish(self._subscribeChannel, JSON.stringify(payload));
                }
            });
...
EventstoreWithProjection.prototype._getChannel = function(query) {
    var channel = (typeof(query) === 'string' ?
        `::${query}` :
...
EventstoreWithProjection.prototype._sleepUntilQueryIsNotified = async function(query, sleepUntil, minRev) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const _channel = this._getChannel(query);
            token = PubSub.subscribe(_channel, function(channel, revision) {
                if (channel === _channel && !isNaN(revision) && revision >= minRev) {
                    clearTimeout(timeoutHandle);
                    PubSub.unsubscribe(token);
                    resolve(true);
                }
...
EventstoreWithProjection.prototype._initPlaybackList = async function(projection) {
    ...
    if (this.options.EventstorePlaybackList && projection.playbackList && this.options.playbackListStore) {
        // const EventstorePlaybackList = require('./eventstore-playback-list');
        ...
        debug('playbacklist not initialized using projection and options', projection, this.options);
    }
};
