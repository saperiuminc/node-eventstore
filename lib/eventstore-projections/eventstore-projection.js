...
class EventstoreWithProjection extends Eventstore {
        ...
        const _init  = async function (params) {
            ...
            for (let index = 0; index < allProjections.length; index++) {
                ...
                if (!self._projections[projection.projectionId]) {
                    debugC('ES-SHARD', self.shard, '_startAllProjections: projecting', projection.projectionId);
    ...
    stopAllProjections(callback) {
        ...
        debugC('ES-SHARD', this.shard, 'stopAllProjections called');
        // just preserving that adrai's public functions are callback based but internally we can just use async/await for elegance
    ...
    project(projectionConfig, callback) {
        debugC('ES-SHARD', this.shard, 'projection called with params:', projectionConfig);
        // just preserving that adrai's public functions are callback based but internally we can just use async/await for elegance
    ...
    getProjectionTasks(projectionId, done) {
        ...
        if (typeof projectionId === 'function') {
            ...
            debugC('CLUSTERED getProjectionTasks: no id');
            this._getProjectionTasks().then((projectionTasks) => {
                callback(null, projectionTasks);
            }).catch(callback);
        } else {
            callback = done;
            debugC('CLUSTERED getProjectionTasks:', projectionId);
    ...
    ***************************************************************************************************/
    ...
    async _deleteProjection(projectionId) {
        try {
            debugC('CLUSTERED: _deleteProjection: ', projectionId);
            const projectionTaskIds = [];
            const projectionTasks = await this._projectionStore.getProjectionTasks(projectionId);
            ...
            debugC('CLUSTERED: _deleteProjection DONE: ', projectionId);
            return projectionTaskIds;
        } catch (error) {
    ...
    async processProjectionTasks(projectionTaskIds, rebalanceId) {
        ...
        debugC('ES-SHARD', self.shard, 'REBALANCE CALLED', projectionTaskIds);
        ...
        _.each(projectionTaskIds, async function(projectionTaskId) {
            while (self._currentAssignments.find((item) => item == projectionTaskId) && self._currentRebalanceId === rebalanceId) {
                try {
                    ...
                    debugC('ES-SHARD', self.shard, 'FOUND PROJECTION TASK:', projectionTaskId);
                    if (projectionTask) {
                        ...
                        try {
                            ...
                            while (continueJob) {
                                try {
                                    ...
                                    debugC('ES-SHARD', self.shard, 'PROCESSING PROJECTION', projectionTaskId);
                                    ...
                                    if (!continueJob) {
                                        debugC('ES-SHARD', self.shard, 'NO LONGER CONTINUING JOB', projectionTaskId);
                                    }
                                }
                            };
                        } catch (error) {
            ...
            debugC('ES-SHARD', self.shard, 'JOB COMPLETE', projectionTaskId);
        });
    }
        ...
        try {
            ...
            if (projectionTask) {
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    debugC('ES-SHARD', this.shard, 'RUNNING TASK', projectionTaskId, projectionTask.partition);
                    ...
                    debugC('ES-SHARD', this.shard, 'PROJECTION TASK GET EVENTS AT OFFSET', offset, projectionTaskId, projectionTask.partition);
                    ...
                    // if (eventsCount) {
                        // debugC('ES-SHARD', this.shard, 'PROJECTION TASK GOT EVENTS AT OFFSET', offset, projectionTaskId, events.length);
                    // } else {
                        // debugC('ES-SHARD', this.shard, 'PROJECTION TASK NO EVENTS AT OFFSET', offset, projectionTaskId);
                    // }
                    ...
                    debugC('ES-SHARD', this.shard, 'DONE RUNNING TASK', projectionTaskId);
                } else {
                    debugC('ES-SHARD', this.shard, 'NOT RUNNING TASK', projectionTaskId);
                }
            } else {
                debugC('ES-SHARD', this.shard, 'NO TASK', projectionTaskId);
            }
        } catch (error) {
     ...
     * @param {string} projectionId 
     * @returns {Promise<ProjectionTask>}
     */
    ...
    async _project(projectionConfig, shouldUpdateProjectionStore) {
        try {
            ...
            try {
                debugC('ES-SHARD', this.shard, 'AWAITING LOCK ON KEY', lockKey);
                lockToken = await this._distributedLock.lock(lockKey, 1900);
                debugC('ES-SHARD', this.shard, 'DONE AWAITING LOCK ON KEY', lockKey);
            } catch (error) {
            ...
            if (shouldUpdateProjectionStore) {
                ...
                if (storedProjection) {
                    ...
                    debugC('ES-SHARD', this.shard, 'UPDATED PROJECTION', projectionConfig.projectionId);
                } else {
                    await this._projectionStore.createProjection(projection);
                    debugC('ES-SHARD', this.shard, 'CREATED PROJECTION', projectionConfig.projectionId);
                }

                // abtract method
                projectionTasks = await this.createProjectionTasks(projection.projectionId);
            } else {
                debugC('ES-SHARD', this.shard, 'shouldUpdateProjectionStore false', projectionConfig.projectionId);
            }
            ...
            for (const projectionTask of projectionTasks) {
                ...
                debugC('ES-SHARD', this.shard, 'SETTING TASKS IN MEMORY', projectionConfig.projectionId);
            }
            ...
            debugC('ES-SHARD', this.shard, '_PROJECT DONE', projectionConfig.projectionId);
        } catch (error) {
