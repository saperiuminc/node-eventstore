...
class EventstoreWithProjection extends Eventstore {
    constructor(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore) {
        ...
        if (options.enableProjection) {
            ...
            if(!self.options.shouldSkipSignalOverride) {
    ...
    project(projectionConfig, callback) {
        debug('ES-SHARD', this.options.shard, 'projection called with params:', projectionConfig);
        // just preserving that adrai's public functions are callback based but internally we can just use async/await for elegance
    ...
    async _runProjection(projectionId, forced) {
        ...
        try {
            ...
            for(const projectionTask of projectionTasks) {
                ...
                if (forced) {
                    await this._projectionStore.setOffset(projectionTaskId, projectionTask.errorOffset ? projectionTask.errorOffset.toString() : null);
                }
    ...
    ***************************************************************************************************/
    ...
    async _deleteProjection(projectionId) {
        try {
            console.log('_deleteProjection: ', projectionId);
            const projectionTaskIds = [];
            const projectionTasks = await this._projectionStore.getProjectionTasks(projectionId);
            console.log('_deleteProjection: tasks', projectionTasks);
            for(const projectionTask of projectionTasks) {
                const projectionTaskId = projectionTask.projectionTaskId;
                projectionTaskIds.push(projectionTaskId);
    ...
    async _resetProjection(projectionId) {
        try {
            ...
            if (projection) {
                ...
                for(const projectionTask of projectionTasks) {
                    const projectionTaskId = projectionTask.projectionTaskId;
                    await this._projectionStore.setOffset(projectionTaskId, '0');
                }
            }
    ...
    async _processProjection(projectionTaskId) {
        ...
        try {
            ...
            if (projectionTask) {
                ...
                const projectionConfig = projectionTask.projection.configuration;
                let offset = projectionTask && projectionTask.offset ? parseInt(projectionTask.offset) : 0;
                if (isNaN(offset)) {
                    offset = 0;
                }

                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    debug('ES-SHARD', this.options.shard, 'RUNNING TASK', projectionTaskId);
                    ...
                    if (!_.isNil(projectionTask.shard) && !_.isNil(projectionTask.partition)) {
                        if (_.isArray(projectionQuery)) {
                            for (let query of projectionQuery) {
                                ...
                                query.partition = projectionTask.partition;
                            }
                        } else {
                            projectionQuery.shard = projectionTask.shard;
                            projectionQuery.partition = projectionTask.partition;
                        }
                    }
                    ...
                    // if (eventsCount) {
                        // debug('ES-SHARD', this.options.shard, 'PROJECTION TASK GOT EVENTS AT OFFSET', offset, projectionTaskId, events.length);
                    ...
                    // } else {
                        // debug('ES-SHARD', this.options.shard, 'PROJECTION TASK NO EVENTS AT OFFSET', offset, projectionTaskId);
                    // }
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            ...
                            errorFault = error;
                            errorEvent = event;
                            errorOffset = event.eventSequence.toString();
                    ...
                    await this._projectionStore.setProcessed(projectionTask.projectionTaskId, Date.now(), eventsCount > 0 ? offset.toString() : undefined, !(eventsCount > 0));
                    ...
                    debug('ES-SHARD', this.options.shard, 'DONE RUNNING TASK', projectionTaskId);
                } else {
                    debug('ES-SHARD', this.options.shard, 'NOT RUNNING TASK', projectionTaskId);
                }
            } else {
                debug('ES-SHARD', this.options.shard, 'NO TASK', projectionTaskId);
            }
        } catch (error) {
    ...
    async _project(projectionConfig, shouldUpdateProjectionStore) {
        try {
            ...
            let lockToken;
            const lockKey = `${projectionKey}-shard-${this.options.shard}`;
            try {
                debug('ES-SHARD', this.options.shard, 'AWAITING LOCK ON KEY', lockKey);
                lockToken = await this._distributedLock.lock(lockKey, 1900);
                debug('ES-SHARD', this.options.shard, 'DONE AWAITING LOCK ON KEY', lockKey);
            } catch (error) {
                console.info('ES-SHARD', this.options.shard, 'was not able to acquire lock. just continue');
            }
            ...
            if (shouldUpdateProjectionStore) {
                ...
                const storedProjectionTasks = await this._projectionStore.getProjectionTasks(projection.projectionId, this.options.shard);

                if (storedProjectionTasks.length === 0) {
                    const createProjectionTaskPromises = [];
                    const partitions = await this._getPartitionsAsync();
                    for (const partition of partitions) {
                        ...
                        const newProjectionTask = {
                            projectionTaskId: this._getProjectionTaskId(projection.projectionId, partition),
                            projectionId: projection.projectionId,
                            shard: this.options.shard,
                            partition: partition,
                            offset: projectionTaskOffset
                        };
                        debug('ES-SHARD', this.options.shard, 'CREATING PROJECTION TASK', newProjectionTask.projectionTaskId);
            ...
            if (!projectionTasks) {
                projectionTasks = await this._projectionStore.getProjectionTasks(projection.projectionId, this.options.shard);
            }
            for (const projectionTask of projectionTasks) {
    ...
    _getProjectionTaskId(projectionId, partition) {
        return `${projectionId}:shard${this.options.shard}:partition${partition}`;
    }

    async _getPartitionsAsync() {
        return new Promise((resolve, reject) => {
            this.store.getPartitions(function(err, partitions) {
                if (err) {
                    reject(err)
                } else {
                    resolve(partitions);
