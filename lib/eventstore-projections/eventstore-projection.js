            ...
            while (currentAssignments.find((item) => item == projectionId) && currentRebalanceId === rebalanceId) {
                ...
                if (projection) {
                    ...
                    try {
                        ...
                        const waitForSubscriptionMessages = function(callback) {
                            if (subscriptionTokens.length == 0) {
                                ...
                                for (const q of queryArray) {
                                    ...
                                    const subToken = pubsub.subscribe(streamId, function(err, message) {
                        ...
                        while (continueJob) {
                            try {
                                if (tryContinueProcessProjection == false) {
                                    ...
                                    await Promise.race([waitForAnySubscriptions, waitTimeout]);
                                }
