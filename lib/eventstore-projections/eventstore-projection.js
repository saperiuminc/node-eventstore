...
const EventPollingSubscription = require('../eventPollingSubscription');
const v8 = require('v8');
...
function EventstoreWithProjection(options, store) {
    ...
    self._eventPollingSubscriptionPool = {};
    self._tokenChannels = {};
    ...
    self._streamBufferLRUCleaner = async.queue(function(task, callback) {
        ...
        const eventPollingSubscription = self._eventPollingSubscriptionPool[channel];
        if (eventPollingSubscription && eventPollingSubscription.isActive()) {
            debug('Clearing Stream Buffer only', channel);
            // There's an active polling subscription, do not cleanup the entire stream buffer. Instead, clear its contents only.
            ...
            // There is no active polling subscriptions. The stream buffer can safely be closed.
            debug('Closing the Stream Buffer', channel);
            self._streamBuffers[channel].close();

            // Close the eventPollingSubscription also if it's defined but not active
            if (eventPollingSubscription) {
                eventPollingSubscription.close();

                delete self._eventPollingSubscriptionPool[channel];
            }
        }
        callback();
    });
...
EventstoreWithProjection.prototype.deactivatePolling = function() {
    const pollers = Object.values(this._eventPollingSubscriptionPool);
...
EventstoreWithProjection.prototype.subscribe = function(query, revision, onEventCallback, onErrorCallback) {
    ...
    try {
        ...
        if (!self._eventPollingSubscriptionPool[channel]) {
            const pollerOptions = {
                es: self,
            ...
            debug('CREATING NEW EventPollingSubscription FOR: ', channel);
            self._eventPollingSubscriptionPool[channel] = new EventPollingSubscription(pollerOptions);
        } else {
            debug('FOUND EXISTING EventPollingSubscription FOR: ', channel);
        }
        const eventPollingSubscription = self._eventPollingSubscriptionPool[channel];

        eventPollingSubscription.subscribe(token, revision, onEventCallback, onErrorCallback);
...
EventstoreWithProjection.prototype.unsubscribe = function(token) {
    try {
        if (this._tokenChannels[token]) {
            ...
            const eventPollingSubscription = this._eventPollingSubscriptionPool[channel];
            if (eventPollingSubscription) {
                eventPollingSubscription.unsubscribe(token);
                return true;
            }
            return false;
        } else {
 ...
 * @returns {void} - returns void
 */
EventstoreWithProjection.prototype._setupNotifyPubSub = async function() {
    return new Promise((resolve, reject) => {
        ...
        try {
            if (self.options.redisConfig && self.redisSub == undefined && self.redisPub == undefined) {
                    ...
                    self.commit = function(eventstream, callback) {
                        ...
                        callback = self._enhanceCallback(callback, function() {
                            ...
                            if (query) {
                                ...
                                if(self.redisPub) {
                                    timing("commit monkeypatch: ", self.options.notifyCommitRedisChannel, data, new Date());
            ...
            return reject(new Error('error on _setupNotifyPubSub' + error));
        }
    });
}
...
EventstoreWithProjection.prototype._processReceivedRedisMessage = function(redisChannel, strData) {
    ...
    if (redisChannel == self.options.notifyCommitRedisChannel) {
        ...
        timing("_processReceivedRedisMessage notifyCommitRedisChannel: ", fullChannel, streamChannel, new Date());
    } else if (redisChannel == self.options.notifyCompletedJobRedisChannel) {
        if (data.token) {
            timing("_processReceivedRedisMessage notifyCompletedJobRedisChannel unsubcribe: ", data.token, new Date());
...
//     return self._setupNotifyPubSub()
//     .then(() => {
//         return new Promise((resolve, reject) => {
//             try {
//                 let token;
