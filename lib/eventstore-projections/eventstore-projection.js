                                ...
                                try {
                                    ...
                                    if (tryContinueProcessProjectionTask == false) {
                                        ...
                                        if (!_.isEmpty(signalResult.messages)) {
                                            const event = JSON.parse(signalResult.messages[0]);
                                            console.log(event);
                                            console.log(projectionTask);

                                            const eventOffset = {
                                                eventId: event.eventId,
                                                streamRevision: event.streamRevision,
                                                commitStamp: event.commitStamp
                                            };

                                            const serializedEventOffset = self._offsetManager.serializeOffset(eventOffset);
                                            if (projectionTask.offset != null && self._offsetManager.compareOffset(serializedEventOffset, projectionTask.offset) < 0) {
                                                // Check if signalled event is already processed where to check against?
                                                const isEventProcessed = true;
                                                if (!isEventProcessed) {
                                                    console.log('projectionTask offset reset to signalled event offset');
                                                    projectionTask.offset = serializedEventOffset;
                                                }
                                            }
                                        }
                                    }
                                    debugC('ES: PROCESSING PROJECTION TASK', projectionTaskId);
                                    const eventsCount = await self._processProjectionTask(projectionTaskId);
            ...
            if (projectionTask) {
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        console.log(event);
                        console.log('CHECK EVENT HERE IN LOOP');
                        try {
                            // Check for gap here
                    ...
                    await this._projectionStore.setProcessed(projectionTask.projectionTaskId, Date.now(), eventsCount > 0 ? offset : undefined, !(eventsCount > 0));
