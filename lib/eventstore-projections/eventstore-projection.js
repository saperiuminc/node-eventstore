...
class EventstoreWithProjection extends Eventstore {
    constructor(opts, dSignal, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore, outputsTo) {
        super(opts, store);
        const self = this;
    ...
    async _runProjection(projectionId, forced) {
        ...
        try {
            ...
            const self = this;
            for (const projectionTask of projectionTasks) {
                const channels = self._getProjectionTaskChannels(projectionTask);

                await self._dSignal.xsubscribe(projectionTask.projectionTaskId, channels, async (err, projectionTaskId) => {
                    await self._processProjectionTask(projectionTaskId)
                });
            }
    ...
    async _deleteProjection(projectionId) {
        try {
            ...
            for(const projectionTask of projectionTasks) {
                ...
                const channels = this._getProjectionTaskChannels(projectionTask);
                await this._dSignal.xunsubscribe(projectionTaskId, channels);
            }
    ...
    ***************************************************************************************************/
    ...
    /**
         * @param {ProjectionTask} projectionTask 
         * @returns {Array<string>}
         */
    _getProjectionTaskChannels(projectionTask) {
        const self = this;
        const projectionConfig = projectionTask.projection.configuration;
