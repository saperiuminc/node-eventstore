...
class EventstoreWithProjection extends Eventstore {
    ...
    getProjectionTasks(projectionId, done) {
        ...
        if (typeof projectionId === 'function') {
            ...
            console.log('getProjectionTasks: no id');
            this._getProjectionTasks().then((projectionTasks) => {
                callback(null, projectionTasks);
            }).catch(callback);
        } else {
            callback = done;
            console.log('getProjectionTasks:', projectionId);
    ...
    async _runProjection(projectionId, forced) {
        console.log('_runProjection called with parameters', projectionId, forced);
        try {
            const projectionTasks = await this._projectionStore.getProjectionTasks(projectionId);
            const projectionTaskIds = [];
            console.log('_runProjection: found tasks', projectionTasks);
            ...
            console.log('_runProjection tasks', projectionTaskIds);
            if (this._taskGroup) {
    ...
    ***************************************************************************************************/
    ...
    async _deleteProjection(projectionId) {
        try {
            ...
            // TODO: Feb2: Assume delete all projection tasks here, or let es call delete for each task separately?
        ...
        _.each(updatedAssignments, async function(projectionTaskId) {
            while (self._currentAssignments.find((item) => item == projectionTaskId) && self._currentRebalanceId === rebalanceId) {
                try {
                    ...
                    // console.log('FOUND PROJECTION TASK:', projectionTask);
                    if (projectionTask) {
                        ...
                        try {
                            ...
                            while (continueJob) {
                                try {
                                    ...
                                    // console.log('PROCESSING PROJECTION', projectionTaskId);
        ...
        try {
            ...
            if (projectionTask) {
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    if (eventsCount) {
                        // console.log('PROJECTION TASK', projectionTask);
    ...
    async _project(projectionConfig, shouldUpdateProjectionStore) {
        try {
            ...
            if (shouldUpdateProjectionStore) {
                ...
                if (storedProjection) {
                    // TODO: Feb2: no await? not synchronous?
                    ...
                    const self = this;
                    const getPartitionsAsync = async () => {
                      return new Promise((resolve) => {
                        self.getPartitions((err, partitions) => resolve(partitions));
                      });
                    }
                    const partitions = await getPartitionsAsync();
                    const getLatestOffsetAsync = async (funcPartition) => {
                      return new Promise((resolve) => {
                        self.getLatestOffset(funcPartition, (err, partitions) => resolve(partitions));
                      });
                    }
            ...
            const projectionTasks = await this._projectionStore.getProjectionTasks(projection.projectionId);
            for (const projectionTask of projectionTasks) {
    ...
    getPartitions(callback) {
      this._getPartitionsAsync().then((partitions) => callback(null, partitions)).catch(callback);
    };
    ...
    getLatestOffset(partition, callback) {
      this._getLatestOffsetAsync(partition).then((latestOffset) => callback(null, latestOffset)).catch(callback);
    };
