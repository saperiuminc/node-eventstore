...
class EventstoreWithProjection extends Eventstore {
    constructor(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore) {
        ...
        this.options = Object.assign(_.clone(defaultOptions), opts)

        self._playbackListViews = {};

        if (this.options.enableProjection) {
            if (!this.options.projectionGroup) {
                throw new Error('projectionGroup is required with enableProjection true');
            }

            self._outputEventstore = this.options.outputsTo;
            self._context = this.options.context;
            self._commitRedisChannel = `context:${this.options.context}:channel:commited`;
            self._completedJobRedisChannel = `projection-group:${this.options.projectionGroup}:channel:completed`
            ...
            this._waitableFactory = new WaitableFactory({
                createRedisClient: this.options.redisCreateClient
            });
    ...
    init(callback) {
        ...
        const _init  = async function (params) {
            if (self.options.enableProjection && self.options.projectionStore) {
                const allProjections = await self._getProjections(self._context);
                for (let index = 0; index < allProjections.length; index++) {
                    const projection = allProjections[index];
                    if (!self._projections[projection.projectionId]) {
                        debugC('ES: _startAllProjections: projecting', projection.projectionId);
                        await self._project(projection.configuration, false);
                    }
                }
            }
        }
    ...
    defineEventMappings(mappings) {
        ...
        const combinedMapping = Object.assign(_.clone(this._requiredEventMappings), mappings);
    ...
    async _project(projectionConfig, shouldUpdateProjectionStore) {
        debugC('ES: _PROJECT called with params:', projectionConfig, shouldUpdateProjectionStore);
        try {
            ...
            try {
                debugC('ES: AWAITING LOCK ON KEY', lockKey);
                lockToken = await this._distributedLock.lock(lockKey, 1900);
                debugC('ES: DONE AWAITING LOCK ON KEY', lockKey);
            } catch (error) {
                console.info('ES: was not able to acquire lock. just continue');
            }
            ...
            if (shouldUpdateProjectionStore) {
                ...
                if (storedProjection) {
                    ...
                    debugC('ES: UPDATED PROJECTION', projection.projectionId);
                } else {
                    await this._projectionStore.createProjection(projection);
                    debugC('ES: CREATED PROJECTION', projection.projectionId);
                }

                // abtract method
                projectionTasks = await this.createProjectionTasks(projection);
            } else {
                debugC('ES: shouldUpdateProjectionStore false', projection.projectionId);
            }

            // set projection task to private list of projection tasks
            if (!projectionTasks) {
                projectionTasks = await this._projectionStore.getProjectionTasks(projection.projectionId);
            }
            for (const projectionTask of projectionTasks) {
                this._projectionTasks[projectionTask.projectionTaskId] = projectionTask;
                debugC('ES: SETTING TASKS IN MEMORY', projectionTask.projectionTaskId);
            }
            ...
            debugC('ES: _PROJECT DONE', projectionConfig.projectionId);
        } catch (error) {
    ...
    /**
     * Abstract method.  Needs to be implemented by concrete classes
     * @param {string} projectionId 
     * @returns {Promise<Array<ProjectionTask>>}
     */
    async createProjectionTasks(projectionId) {
     ...
     * @returns {void} - returns void
     */
    runProjection(projectionId, forced, callback) {
    ...
    /**
     * @param {string} projectionId
     * @returns {Promise<Array<string>>} - returns an array of projection task ids that were successfully run
     */
    async _runProjection(projectionId, forced) {
        debugC('ES: _runProjection called with parameters', projectionId, forced);
        try {
            ...
            debugC('ES: _runProjection projection task ids', projectionTaskIds);
            if (this._taskGroup) {
    ...
    startAllProjections(callback) {
        debugC('ES: startAllProjections called');
    ...
    async _startAllProjections() {
        try {
            debugC('ES: _startAllProjections called');

            if (this.options.enableProjection === false) {
                throw new Error('enableProjection true is required with startAllProjections');
            }

            debugC('ES: _startAllProjections: doing task assignment');
            const projectionTasks = await this._getProjectionTasksByContext(this._context);

            this.doTaskAssignment(this.options.projectionGroup, projectionTasks);

            debugC('ES: _startAllProjections done');
        } catch (error) {
    ...
    /**
     * 
     * @param {string} projectionGroup 
     * @param {ProjectionTask[]} projectionTasks 
     */
    ...
    /**
     * 
     * @param {Array<string>} projectionTaskIds 
     * @param {string} rebalanceId 
     */
    async processProjectionTasks(projectionTaskIds, rebalanceId) {
        const self = this;
        self._currentAssignments = projectionTaskIds;
        self._currentRebalanceId = rebalanceId;

        debugC('ES: REBALANCE CALLED', projectionTaskIds);
        self.emit('rebalance', projectionTaskIds);

        _.each(projectionTaskIds, async function(projectionTaskId) {
            while (self._currentAssignments.find((item) => item == projectionTaskId) && self._currentRebalanceId === rebalanceId) {
                try {
                    ...
                    debugC('ES: FOUND PROJECTION TASK:', projectionTaskId);
                    if (projectionTask) {
                        ...
                        try {
                            ...
                            let tryContinueProcessProjectionTask = false;
                            ...
                            while (continueJob) {
                                try {
                                    ...
                                    let signaledMessages;
                                    if (tryContinueProcessProjectionTask == false) {
                                        const signalResult = await waitableConsumer.waitForSignal(self.options.pollingTimeout);
                                        signaledMessages = signalResult.messages;
                                    }
                                    debugC('ES: PROCESSING PROJECTION TASK', projectionTaskId);
                                    const eventsCount = await self._processProjectionTask(projectionTaskId, signaledMessages);
                                    tryContinueProcessProjectionTask = (eventsCount == self.options.pollingMaxRevisions);
                                } catch (error) {
                                    console.error('error in while loop in shared job', error);
                                    continueJob = false;
                                    tryContinueProcessProjectionTask = false;
                                } finally {
                                    continueJob = (self._currentAssignments.find((item) => item == projectionTaskId) && self._currentRebalanceId === rebalanceId);
                                    if (!continueJob) {
                                        debugC('ES: NO LONGER CONTINUING JOB', projectionTaskId);
                                    }
                                }
                            };
                        } catch (error) {
            ...
            debugC('ES: JOB COMPLETE', projectionTaskId);
        });
    }

    /**
     * @param {String} projectionTaskId the job to process
     * @param {Array<Event>} signaledEvents the events that triggered the projection to process 
     * @returns {Promise<Number>} returns number of events processed
     */
    async _processProjectionTask(projectionTaskId, signaledEvents) {
        const self = this;
        // let eventsCount = 0;
        ...
        try {
            ...
            if (projectionTask) {
                ...
                let offset = projectionTask.offset ? projectionTask.offset : self.store.getOffsetManager().getStartingSerializedOffset(hasPartitions);

                // console.log(signaledEvents);
                if (signaledEvents && signaledEvents.length > 0) {
                    const events = _.cloneDeep(signaledEvents);
                    events.sort((event1, event2) => {
                        return self.store.getOffsetManager().compareEventOffset(event1, event2);
                    });

                    const earliestEvent = events[0];
                    if (!_.isNil(projectionTask.offset) && self.store.getOffsetManager().compareEventWithOffset(earliestEvent, projectionTask.offset) < 0) {
                        // TODO: Optimization review:
                        // Checking if processed here can prevent more duplicates before proceeding to processProjectionTask
                        // eventFromSignal = earliestEvent;

                        // Check for gap here, rename to getDeduplicationCheckpoint
                        // const projectionStreamVersion = await this._projectionStore.getProjectionStreamVersion(projectionTask.projectionId, earliestEvent.context, earliestEvent.aggregate, earliestEvent.aggregateId);

                        // console.log('===========');
                        // console.log('projectionStreamVersion');
                        // console.log(projectionStreamVersion);
                        // console.log('event');
                        // console.log(event);
                        // console.log('===========');
                        // TODO: what if projectionStreamVersion is null?
                        // TODO: i added a check if projectionStreamVersion is null. if null then process the event
                        // if (earliestEvent.streamRevision > projectionStreamVersion || !projectionStreamVersion) {
                            offset = self.store.getOffsetManager().computeEarliestOffset(earliestEvent, offset);
                        // }
                    }
                }
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    debugC('ES: RUNNING TASK', projectionTaskId, projectionTask.partition);
                    ...
                    if (!_.isNil(projectionTask.shard) && !_.isNil(projectionTask.partition)) {
                        if (_.isArray(projectionQuery)) {
                            for (let query of projectionQuery) {
                                ...
                                query.partition = hasPartitions ? projectionTask.partition : undefined;
                            }
                        } else {
                            projectionQuery.shard = projectionTask.shard;
                            projectionQuery.partition = hasPartitions ? projectionTask.partition : undefined;
                        }
                    }

                    // console.log('ES: PROJECTION TASK GET EVENTS AT OFFSET', offset, projectionTaskId, projectionTask.partition);
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            ...
                            // console.log('===========');
                            // console.log('projectionStreamVersion', projectionTask.projectionTaskId, event.context, event.aggregate, event.aggregateId, event.streamRevision, projectionStreamVersion);
                            // console.log(projectionStreamVersion);
                            // console.log('event');
                            // console.log(event);
                            // TODO: what if projectionStreamVersion is null?
                            // TODO: i added a check if projectionStreamVersion is null. if null then process the event
                            if (event.streamRevision > projectionStreamVersion || _.isNil(projectionStreamVersion)) {
                                eventsForCheckpoint.push(event);
                                await this._playbackEvent(event, projectionConfig, projectionConfig.eventCallbackTimeout || this.options.eventCallbackTimeout);
                                lastProcessedOffset = event.eventSequence;

                                // Set projection checkpoint here
                                ...
                                // const newProjectionStreamVersion = await this._projectionStore.getProjectionStreamVersion(projectionTask.projectionTaskId, event.context, event.aggregate, event.aggregateId);
                                processedEventsCount++;
                                // console.log('newProjectionStreamVersion', projectionTask.projectionTaskId, event.context, event.aggregate, event.aggregateId, event.streamRevision, newProjectionStreamVersion);
                            }
                        } catch (error) {
                    ...
                    // const eventsByStream = _.groupBy(eventsForCheckpoint, (event) => {
                    //     return `${event.context}-${event.aggregate}-${event.aggregateId}`;
                    // });

                    // const lastEventByStreams = {};
                    // _.forOwn(eventsByStream, (events, key) => {
                    //     lastEventByStreams[key] = events[events.length - 1];
                    // });

                    // set to processed if no errors OR if we are not stopping if there is a fault
                    // console.log('Set processed!!', projectionTask.projectionTaskId, Date.now(), processedEventsCount > 0 ? lastProcessedOffset : undefined, !(processedEventsCount > 0))
                    await this._projectionStore.setProcessed(projectionTask.projectionTaskId, Date.now(), processedEventsCount > 0 ? lastProcessedOffset : undefined, !(processedEventsCount > 0));
                    ...
                    debugC('ES: DONE RUNNING TASK', projectionTaskId);
                } else {
                    debugC('ES: NOT RUNNING TASK', projectionTaskId);
                }
            } else {
                debugC('ES: NO TASK', projectionTaskId);
            }
        } catch (error) {
    ...
    async _stopAllProjections() {
        debugC('ES: stopAllProjections called');
        try {
    ...
    getProjectionTasks(projectionId, done) {
        ...
        if (typeof projectionId === 'function') {
            ...
            debugC('ES: getProjectionTasks: no id filter');
            this._getProjectionTasks().then((projectionTasks) => {
                callback(null, projectionTasks);
            }).catch(callback);
        } else {
            callback = done;
            debugC('ES: getProjectionTasks:', projectionId);
    ...
    pauseProjection(projectionId, done) {
        this._pauseProjection(projectionId).then((data) => done(null, data)).catch(done);
    };
    ...
    async _pauseProjection(projectionId) {
        try {
            debugC('ES: _pauseProjection: ', projectionId);
            
            await this._projectionStore.setState(projectionId, 'paused');

            debugC('ES: _pauseProjection DONE: ', projectionId);
        } catch (error) {
            console.error('_pauseProjection failed with parameters and error', projectionId, error);
            throw error;
        }
    };
        ...
        try {
            ...
            if (projection) {
                ...
                for(const projectionTask of projectionTasks) {
                    const projectionTaskId = projectionTask.projectionTaskId;
                    let offset = null;
                    await this._projectionStore.setOffset(projectionTaskId, offset);
                }
            }
    ...
    async _deleteProjection(projectionId) {
        try {
            debugC('ES: _deleteProjection: ', projectionId);
            const projectionTaskIds = [];
            const projectionTasks = await this._projectionStore.getProjectionTasks(projectionId);
            ...
            debugC('ES: _deleteProjection DONE: ', projectionId);
            return projectionTaskIds;
        } catch (error) {
            console.error('_deleteProjection failed with parameters and error', projectionId, error);
            throw error;
        }
    };

    getChannelsToSignal(query) {
        throw new Error('getChannelsToSignal must be implemented!');
    }
     ...
     */
    sendSignal(query, message, done) {
        const self = this;
        const partition = this.store.getPartitionId(query.aggregateId);
        const channels = self.getChannelsToSignal(query);
        for (const channel of channels) {
            self._waitableProducer.signal(channel, message);
        }
    ...
    ***************************************************************************************************/
     ...
     * @returns {Promise<Array<ProjectionTask>} returns Promise of type void
     */
       async _getProjectionTasksByContext(context) {
        try {
            return this._projectionStore.getProjectionTasksByContext(context);
        } catch (error) {
            console.error('_getProjectionTasksByContext failed with parameters and error', error);
            throw error;
        }
    };
    ...
    _getProjectionStateStreamId(projectionConfig, lastProjectionEvent) {
        ...
        if (projectionConfig.partitionBy === 'stream') {
            ...
            // format: <projectionid>-<aggregateId>-result
     ...
     * @returns {Promise<void>} returns a Promise of type void
     */
    async _getLatestOffsetAsync(shard, partition) {
        return new Promise((resolve, reject) => {
            this.store.getLatestOffset(shard, partition, (error, latestOffset) => {
                if (error) {
                    reject(error);
                }
