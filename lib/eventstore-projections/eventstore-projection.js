...
EventstoreWithProjection.prototype._stopAllProjections = async function() {
    try {
        ...
        for (let index = 0; index < allProjections.length; index++) {
            ...
            await this._stopWaitingForProjectionSignals(projection.projectionId);
        }
    } catch (error) {
        console.error('error in _startAllProjections with params and error:', error);
        throw error;
...
EventstoreWithProjection.prototype._doLeaderElection = async function(projectionGroup) {
    ...
    this._isLeaderElectionActive = true;
    ...
    try {
        ...
        while (continueJob && this._isLeaderElectionActive) {
            try {
        ...
        if (this._isLeaderElectionActive) {
 ...
 * 
 */
EventstoreWithProjection.prototype._undoLeaderElection = async function() {
    // TODO: use distributed lock injected object for testing compliance
    this._isLeaderElectionActive = false;
};
        ...
        if (projection) {
            if (projection.state == 'running') {
                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    try {
                        await this.playbackEvent(event, projectionConfig, this.options.eventCallbackTimeout);
 ...
 * @returns {Promise<void>} returns a Promise that resolves to a void
 */
EventstoreWithProjection.prototype.playbackEvent = function(event, projectionConfig, timeout) {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            console.error('error in playbackEvent with params and error', error);
            console.error(JSON.stringify(event));
            console.error(JSON.stringify(projectionConfig));
            reject(error);
        }
...
EventstoreWithProjection.prototype._startWaitingForSignals = async function(projectionId) {
    ...
    const subscriptionId = await this._distributedSignal.groupSubscribe(projectionId, this.options.projectionGroup, this.options.pollingTimeout, function(err, done) {
