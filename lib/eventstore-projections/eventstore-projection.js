...
EventstoreWithProjection.prototype.subscribe = function(query, revision, onEventCallback, onErrorCallback) {
    ...
    try {
        ...
        // note: notify only; no guarantee, only a clue
        // possible unordered, or data lost
        // self._streamBuffersSubscriptions[channel] = PubSub.subscribe(channel, function(redisChannel, data) {
        //     if (self._streamBuffers[channel]) {
        //         self._streamBuffers[channel].offerEvent(data);
        //     }
        // });

        // async buffered event subscription
        // self._doPollingSubscription(token, targetQuery, revision, onEventCallback);
