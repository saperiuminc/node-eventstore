...
function EventstoreWithProjection(options, store) {
    ...
    /**
     * @type {Object.<string, StreamBuffer}
     */
    self._streamBuffers = {};
    ...
    try {
        ...
        if (!self._streamBuffers[channel]) {
            ...
            const options = {
                ...
                channel: channel,
                bucket: bucket,
                capacity: STREAM_BUFFER_MAX_CAPACITY,
            ...
            self._streamBufferBuckets[bucket][channel] = now;
        }
...
EventstoreWithProjection.prototype._onStreamBufferEventOffered = function(currentBucket, channel) {
    ...
    if (heapPercentage >= STREAM_BUFFER_MEMORY_THRESHOLD) {
        ...
        const bucketToClean = sortedBuckets[sortedBuckets.length - 1];
        const channels = Object.keys(bucketToClean);
    ...
    this._streamBufferBuckets[newBucket][channel] = now;
}

/**
 * Gets the current stream buffer bucket based on the current time.
 * @returns {String} returns the string representation of the bucket
 */
