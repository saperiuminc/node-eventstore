 ...
 * @returns {Promise<void>} returns a Promise that resolves to a void
 */
EventstoreWithProjection.prototype._getFunctionArgs = function(func) {
    // REF: https://davidwalsh.name/javascript-arguments
            ...
            if (eventHandlerFunc) {
                if (projection.outputState === 'true') {
                    this._getProjectionState(projection, event).then((projectionState) => {
                        ...
                        const doneCallback = function(error) {
                            ...
                            if (error) {
                                ...
                                if (!_.isEqual(projectionState.state, mutableState)) {
                                    ...
                                    self._saveProjectionState(projection.projectionId, projectionState.id, mutableState, this._context).then(resolve).catch(reject);
                                } else {
                        ...
                        const result = eventHandlerFunc(mutableState, event, self._getPlaybackFunctions(mutableState), doneCallback);
                    ...
                    const result = eventHandlerFunc(null, event, self._getPlaybackFunctions(), doneCallback);
                    if (result instanceof Promise) {
                        return result.then(doneCallback).catch(doneCallback);
                    }
                }
