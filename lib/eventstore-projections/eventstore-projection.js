 ...
 * @property {String} eventNameFieldName the field name of the event's name in the payload
 */
    ...
    try {
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            try {
                ...
                await this._playbackEvent(event, projection.playbackInterface, this.options.pollingTimeout);
            } catch (error) {
 ...
 * @param {ProjectionPlaybackInterface} playbackInterface the object/interface to use to playback the event
 ...
 */
EventstoreWithProjection.prototype._playbackEvent = async function(event, playbackInterface, timeout) {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const eventFunc = playbackInterface[eventName];
            eventFunc(event, funcs, (error) => {
