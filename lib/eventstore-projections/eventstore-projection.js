...
const DeduplicationManager = require('../deduplication-manager');
const SimpleEventSubscriptionQueryManager = require('../event-subscription-query-managers/simple-event-subscription-query-manager');
const StreamBufferedEventSubscriptionQueryManager = require('../event-subscription-query-managers/stream-buffered-event-subscription-query-manager');
...
class EventstoreWithProjection extends Eventstore {
    constructor(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore, outputsTo) {
        ...
        if (this.options.enableProjection) {
            ...
            self._deduplicationManager = new DeduplicationManager(self._projectionStore, self.store.getOffsetManager());
            ...
            self._eventSubscriptionPool = {};
            self._tokenChannels = {};

            let eventSubscriptionQueryManager = new SimpleEventSubscriptionQueryManager({
                es: self
            });
            if (self.options.enableSubscriptionEventStreamBuffer) {
                let streamBufferedEventSubscriptionQueryManager = new StreamBufferedEventSubscriptionQueryManager({
                    es: self,
                    eventSubscriptionQueryManager: eventSubscriptionQueryManager
                });
                self.eventSubscriptionQueryManager = streamBufferedEventSubscriptionQueryManager;
            } else {
                self.eventSubscriptionQueryManager = eventSubscriptionQueryManager;
            }
        }
    ...
    async _processProjectionTask(projectionTaskId, signaledEvents) {
        ...
        try {
            ...
            if (projectionTask) {
                ...
                let offset = this._deduplicationManager.getLastProcessedOffset(projectionTask, signaledEvents);
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            const isEventProcessed = await this._deduplicationManager.isEventProcessed(projectionTask, event);
                            if (!isEventProcessed) {
                                ...
                                await this._deduplicationManager.setEventAsProcessed(projectionTask, event);
     ...
     */
    deactivatePolling() {
        const pollers = Object.values(this._eventSubscriptionPool);
    ...
    subscribe(query, revision, onEventCallback, onErrorCallback) {
        ...
        try {
            ...
            if (!self._eventSubscriptionPool[channel]) {
                const pollerOptions = {
                    eventSubscriptionQueryManager: self.eventSubscriptionQueryManager,
                ...
                debug('CREATING NEW EventSubscription FOR: ', channel);
                self._eventSubscriptionPool[channel] = new EventSubscription(pollerOptions);
            } else {
                debug('FOUND EXISTING EventSubscription FOR: ', channel);
            }
            const eventSubscription = self._eventSubscriptionPool[channel];

            eventSubscription.subscribe(token, revision, onEventCallback, onErrorCallback);
            self.eventSubscriptionQueryManager.setChannelActive(channel, true);
        ...
        try {
            if (self._tokenChannels[token]) {
                ...
                if (eventSubscription) {
                    eventSubscription.unsubscribe(token);
                    if (!eventSubscription.isActive()) {
                        self.eventSubscriptionQueryManager.setChannelActive(channel, false);
                        // TODO: Review if possible memory buildup of not cleaning up eventSubscriptionPool
                        // self._eventSubscriptionPool[channel].close();
                        // delete self._eventSubscriptionPool[channel];
                    }
                    return true;
                }
                return false;
