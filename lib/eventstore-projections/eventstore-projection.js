...
class EventstoreWithProjection extends Eventstore {
    constructor(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore, offsetManager) {
        super(opts, store);
        const self = this;
        ...
        if (this.options.enableProjection) {
            ...
            self._offsetManager = offsetManager;
                        ...
                        try {
                            ...
                            while (continueJob) {
                                try {
                                    ...
                                    if (tryContinueProcessProjectionTask == false) {
                                        ...
                                        if (!_.isEmpty(signalResult.messages)) {
                                            ...
                                            events.sort((event1, event2) => {
                                                console.log(self._offsetManager);
                                                return self._offsetManager.compareSerializedOffset(event1.eventSequence, event2.eventSequence);
                                            });

                                            const earliestEvent = events[0];
                                            if (projectionTask.offset != null && self._offsetManager.compareSerializedOffset(earliestEvent.eventSequence, projectionTask.offset) < 0) {
                                    ...
                                    const eventsCount = await self._processProjectionTask(projectionTaskId, eventFromSignal);
     ...
     */
    async _processProjectionTask(projectionTaskId, overrideEvent) {
        const self = this;
        let eventsCount = 0;
        try {
            /**
            ...
            if (projectionTask) {
                if (overrideEvent) {
                    ...
                    projectionTask.offset = self._offsetManager.serializeOffset(offsetToSerialize);
                }
                const projectionConfig = projectionTask.projection.configuration;
                ...
                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !this.options.stopOnFaulted)) {
                    ...
                    // if (eventsCount) {
                    //     debugC('ES: PROJECTION TASK GOT EVENTS AT OFFSET', offset, projectionTaskId, events.length);
                    // } else {
                    //     debugC('ES: PROJECTION TASK NO EVENTS AT OFFSET', offset, projectionTaskId);
                    // }
                    ...
                    for (let i = 0; i < events.length; i++) {
                        ...
                        try {
                            ...
                            // console.log(event);
                            // console.log('===========');
                            if (event.streamRevision > projectionStreamVersion) {
