 ...
 */
 EventstoreProjectionStore.prototype.getProjections = async function(context) {
 ...
 * @returns {Promise<void>} - returns a Promise of type void
 */
 EventstoreProjectionStore.prototype.setError = async function(projectionId, error, errorEvent, errorOffset) {
    const query = `UPDATE ${this.options.name}
                    SET 
                        error = ?,
                        error_event = ?,
 ...
 * @returns {Promise<import('./eventstore-projection').Projection>} - returns a Promise of type Object where object is the Query result
 */
