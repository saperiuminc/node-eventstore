if (process.env.ENABLE_TRACING == 'true') {
    ...
    (function patchJobsManager(proto) {
        shimmer.wrap(proto, 'processJobGroup', function(original) {
            return function() {
                // if no active namespace context yet (run or runAndRetrun has not yet ran) 
                // then run a context
                if (!nsTracer.active) {
                    const spanName = `eventstore-extensions._processJob`;
                    return nsTracer.runAndReturn((context) => {
                        var processJobCallback = arguments[2];
                        arguments[2] = nsTracer.bind(async function() {
                            // processJobCallback is async
                            const span = tracer.startSpan(spanName);
                            nsTracer.set('parentSpan', span);
                            const result = await processJobCallback.apply(this, arguments);
                            span.finish();
    ...
    })(require('./jobs-manager').prototype);

    
    (function patchEventstorePlaybackEvent(proto) {
            ...
            return function() {
                if (nsTracer.active) {
                    return nsTracer.runAndReturn(async (context) => {
                        ...
                        span.setBaggageItem('method', '_playbackEvent');
