...
class TaskAssignmentGroup extends EventEmitter {
    ...
    async _doMemberJob(membershipId) {
        ...
        const memberSubscriptionToken = self._pubsubClient.subscribe(membersStreamId, function(error, message) {
        ...
        const taskSubscriptionToken = self._pubsubClient.subscribe(leaderStreamId, function(error, message) {
            const msg = JSON.parse(message);
        ...
        await this._pubsubClient.unsubscribe(memberSubscriptionToken);
        await this._pubsubClient.unsubscribe(taskSubscriptionToken);
    }
