...
class ClusteredEventStore extends EventstoreWithProjection {
    constructor(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore, offsetManager) {
        super(opts, store, distributedSignal, distributedLock, playbackListStore, playbackListViewStore, projectionStore, stateListStore, offsetManager);
    ...
    getChannelsToSignal(query) {
        let partition = null;
        if (_.isNumber(query.partition) || query.partition === '__unpartitioned') {
          partition = query.partition;
        } else {
          partition = this.store.getPartitionId(query.aggregateId);
        }
        let index = 0;
        let allChannels = []
