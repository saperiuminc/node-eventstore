   ...
   resetOffset(partition) {
     if (partition === '__unpartitioned') { // single concurrency
       ...
       while (index < this._options.numberOfShards) {
          finalOffsetArray.push(0);
          index++;
       }
       return finalOffsetArray;
     } else {
       return 0;
     }
   }
    ...
    formatOffset(rawOffset) {
      if (Array.isArray(rawOffset)) { // single concurrency
        let index = 0;
        const finalOffsetArray = [];
        while (index < this._options.numberOfShards) {
          if (index >= rawOffset.length || isNaN(rawOffset[index])) {
            finalOffsetArray.push(0)
          } else {
            finalOffsetArray.push(rawOffset[index])
          }
          index++;
        }
        return finalOffsetArray;
    ...
    calculateNewOffset(event, oldOffset) {
      ...
      if (Array.isArray(oldOffset)) {
        const shard = helpers.getShard(event.aggregateId, this._options.numberOfShards);
        finalOffset = oldOffset;
        finalOffset[shard] = event.eventSequence || 0;
      } else {
        finalOffset = event.eventSequence || 0
      }
      return finalOffset;
    }
