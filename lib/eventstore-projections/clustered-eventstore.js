...
class ClusteredEventStore extends EventstoreWithProjection {
    ...
    async _doSingleConcurrencyTopology(projection, projectionTasks) {
      const self = this;
      for (const task of projectionTasks) {
        const es = _.find(self._eventstores, (es) => {
            return es.shard == task.shard;
        });
      }
    }

    async _singleConcurrency() {
        // single concurrency
        // get all projections
        const self = this;
        const projections = await self.getProjectionsAsync();
        for(const projection of projections) {
          // handle only projections with null/empty/0 partitionBy
          if (!projection.projectionConfig.partitionBy) {
            let eventStream = null;
            let eventsCount = 0;
            const readableStreamsFromShards = [];
            const writableWriteFuncs = [];
            const finalProcessedFuncs = [];
            // loop through all evenstores/shards to gather the streams needed
            for (const eventstore of self._eventstores) {
              const projectionTask = await eventstore._getProjectionTasks(projection.projectionId);
              if (projectionTask) {
                const projectionConfig = projectionTask.projection.configuration;
                let offset = projectionTask && projectionTask.offset ? projectionTask.offset : 0;
                if (isNaN(offset)) {
                    offset = 0;
                }

                if (projectionTask.projection.state == 'running' || (projectionTask.projection.state == 'faulted' && !eventstore.options.stopOnFaulted)) {
                    let projectionQuery = _.clone(projectionConfig.query);
                    // TODO: another stream for waitForSignal. Listen to the on data of that to activate the streams again
                    // TODO: on inserting another event on a dead stream, pause all other streams and reactivate 
                    readableStreamsFromShards.push(await eventstore._getEventsStreamAsync(eventstore, projectionQuery, offset, eventstore.options.pollingMaxRevisions));

                    const writableWriteForProjection = (writeId, event, callback) => {
                      eventsCount++;
                      // TODO: call playbackEvent on the eventstore/shard which gave the event. Ex. event is from shard1, call shard1._playbackEvent
                      eventstore._playbackEvent(event, projectionConfig, projectionConfig.eventCallbackTimeout || eventstore.options.eventCallbackTimeout).then((data) => {
                        callback(null, data);
                      })
                      .catch((mainError) => {
                        console.error('error in playing back event in partitioned queue with params and error', event, projectionTask.projectionTaskId, mainError);
                        ...
                        const errorFault = mainError;
                        const errorEvent = event;
                        const errorOffset = event.eventSequence;
      
                        // TODO: Feb2 Review aggregate
                        /** @type {AggregateQuery} */
                        const errorStreamQuery = {
                            context: 'projection-errors',
                            aggregate: projectionTask.projectionTaskId,
                            aggregateId: `${projectionTask.projectionId}-errors`
                        }

                        const errorPromises = [];
                        errorPromises.push(eventstore._saveEventAsync(eventstore._outputEventstore, errorStreamQuery, {
                            event: event.payload,
                            error: mainError ? JSON.stringify(mainError, Object.getOwnPropertyNames(mainError)) : null
                        }).catch((error) => {
                        ...
                        errorPromises.push(eventstore._projectionStore.setState(projectionTask.projectionId, 'faulted'));
                        errorPromises.push(eventstore._projectionStore.setError(projectionTask.projectionTaskId, errorFault, errorEvent, errorOffset));
    
                        eventstore.emit('playbackError', errorFault);
      
                        // break if we should stop processing readstream and writestream
                        if (eventstore.options.stopOnFaulted) {
                          eventStream.push(null);
                        }

                        Promise.all(errorPromises).then(callback).catch(callback);
                      })
                    };

                    writableWriteFuncs.push(writableWriteForProjection);


                    const finalProcessing = () => {
                        // TODO: Feb2 Review signature
                        eventstore.emit('playbackSuccess', {
                        ...
                        // TODO: this is used for flushed. Set every 100 events or every 60 seconds or after every event finished
                        eventstore._projectionStore.setProcessed(projectionTask.projectionTaskId, Date.now(), offset, !(eventsCount > 0));
                    }

                    finalProcessedFuncs.push(finalProcessing)
                }
              }
            }
            // at the end of the loop, join the streams
            const joinedStreamsClass = new JoinStreams({
              readableStreams: readableStreamsFromShards,
              writableStreams: [],
              writableWrites: writableWriteFuncs,
              writableFinals: []
            });

            writableWriteFuncs.forEach((item, index) => {
              joinedStreamsClass.addToWritableStreams(index);
            })

            eventStream = joinedStreamsClass.getJoinedReadableStreams();
            const funcsStream = joinedStreamsClass.getJoinedWritableStreams();

            // on finish write, call setProcess
            joinedStreamsClass.on('finishWrite', () => {
              finalProcessedFuncs.forEach((finalProcessing) => {
                finalProcessing();
              })
            });
            
            eventStream.pipe(funcsStream);
    ...
    async _doTaskAssignment(projectionGroup, allProjectionTaskIdsInAllShards) {
        ...
        self._multiTaskGroup = new TaskAssignmentGroup({
            initialTasks: tasks,
            createRedisClient: self._options.redisCreateClient,
            groupId: projectionGroup + `:multi-concurrency`,
            membershipPollingTimeout: self._options.membershipPollingTimeout,
            distributedLock: self._distributedLock
        });

        
        self._singleTaskGroup = new TaskAssignmentGroup({
            initialTasks: tasks,
            createRedisClient: self._options.redisCreateClient,
            groupId: projectionGroup + `:single-concurrency`,
            membershipPollingTimeout: self._options.membershipPollingTimeout,
            distributedLock: self._distributedLock
        });
    
        await self._multiTaskGroup.join();
        // TODO: rename rebalance() function
        self._multiTaskGroup.on('rebalance', async (updatedAssignments, rebalanceId) => {
            console.log('MULTI REBALANCE CALLED', updatedAssignments);
        ...
        await self._singleTaskGroup.join();
        self._singleTaskGroup.on('rebalance', async (updatedAssignments, rebalanceId) => {
            console.log('SINGLE REBALANCE CALLED', updatedAssignments);
            // updatedAssignments => array of projectionTaskId
    ...
    project(projectionConfig, callback) {
      if (projectionConfig.partitionBy == 'stream' || _.isFunction(projectionConfig.partitionBy)) {
        ...
        this._projectSingleConcurrency(projectionConfig, callback);
      }
    }

    async _projectSingleConcurrency(projectionConfig, callback) {
      const self = this;
      let projectionTaskOffset = 0;
      if (projectionConfig && projectionConfig.fromOffset === 'latest') {
        // TODO: need to get/store there in the clustered-eventstore. Must be write model
        // this is the bookmark of all the shards per projection
          projectionTaskOffset = await self._getLatestOffsetAsync();
      }

      const newProjectionTask = {
          projectionTaskId: self._getProjectionTaskId(projectionConfig.projectionId),
          projectionId: projectionConfig.projectionId,
          shard: null,
          offset: projectionTaskOffset
      };
      await self._projectionStore.createProjectionTask(newProjectionTask);
      callback();
    }
