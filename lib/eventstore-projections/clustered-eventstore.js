...
class ClusteredEventStore extends EventstoreWithProjection {
    ...
    _getPartitions(eventstore) {
        return eventstore.store.getPartitions();
    }
    ...
    async _project(projectionConfig) {
        ...
        const promises = [];
        for (const eventstore of this._eventstores) {
            const partitions = this._getPartitions(eventstore);
    ...
    setEventToDispatched(evtOrId, callback) {
        // TODO: Review sharding. EventId can simply be passed so there's no way to determine shard in that case
        if (typeof evtOrId === 'object') {
            evtOrId = evtOrId.id;
            let aggregateId = evtOrId.aggregateId;
            this._doOnShardedEventstore(aggregateId, 'setEventToDispatched', evtOrId, callback);
        } else {
