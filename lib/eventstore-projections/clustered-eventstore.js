...
class ClusteredEventStore extends EventstoreWithProjection {
    ...
    startAllProjections(callback) {
        this._startAllProjections().then((data) => callback(null, data)).catch(callback);
    }

    async _doSingleConcurrencyTopology(projection, projectionTasks) {
        // TODO: Ryan: add code here to get data from multiple shards
        // const projectionConfig = projection.projectionConfig;
        // let topics = [];
        // let queryArray = [];
        ...
        //     const streamId = this._getChannel(q, projection.shard);
        //     topics.push(streamId);
        // }

        // const waitableConsumer = self._waitableFactory.createWaitableConsumer(topics);

        // await waitableConsumer.waitForSignal(self.options.pollingTimeout);

        // waitableConsumer.pipe()
        // inputStream.write()

        for (const task of projectionTasks) {
            // TODO: Ryan: you can already get the offset here so you can query the database
            // task.offset
            // select * from events where id > task.offset
            // select * from events where id > task.offset
            // select * from events where id > task.offset
            // select * from events where id > task.offset
            const es = _.find(this._eventstores, (es) => {
    ...
    async _doMultiConcurrencyTopology(allProjectionTaskIdsInAllShards) {
        await this._doTaskAssignment(this._options.projectionGroup, allProjectionTaskIdsInAllShards)
    }
    ...
    async _startAllProjections() {
        ...
        const projections = this.getProjectionsAsync();

        // TODO: Ryan: Optimize call by doing just one getProjectionTasks and filtering out projection
        let allMultiListenerProjectionTasksInAllShards = [];
        for (const projection of projections) {
            const projectionTasks = await this.getProjectionTasksAsync(projection.projectionId);
            if (projectionConfig.partitionBy == 'stream' || _.isFunction(projectionConfig.partitionBy)) {
                allMultiListenerProjectionTasksInAllShards = allMultiListenerProjectionTasksInAllShards.concat(projectionTasks);
            } else {
                await this._doSingleConcurrencyTopology(projection, projectionTasks);
            }
        }

        const filterByDictionary = {};
        for(const projectionTask of allMultiListenerProjectionTasksInAllShards) {
            filterByDictionary[projectionTask.projectionTaskId] = projectionTask.projectionTaskId;
        }
        allMultiListenerProjectionTasksInAllShards = Object.values(filterByDictionary);

        await this._doMultiConcurrencyTopology(allMultiListenerProjectionTasksInAllShards);
    }
