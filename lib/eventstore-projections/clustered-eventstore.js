...
class ClusteredEventStore extends EventstoreWithProjection {
    ...
    getChannelsToSignal(query) {
        let aggregateShard = null;
        let partition = null;
        if (_.isNumber(parseInt(query.shard || ''))) {
          aggregateShard = query.shard;
        } else {
          aggregateShard = helpers.getShard(query.aggregateId, this._options.numberOfShards);
        }
        if (_.isNumber(query.partition) || query.partition === '__unpartitioned') {
          partition = query.partition;
        } else {
          partition = this.store.getPartitionId(query.aggregateId, aggregateShard);
        }
        let index = 0;
        let allChannels = []
