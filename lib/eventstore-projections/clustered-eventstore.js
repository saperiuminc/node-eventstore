...
class ClusteredEventStore extends EventstoreWithProjection {
    ...
    getChannelsForSignal(query) {
        const aggregateShard = helpers.getShard(query.aggregateId, this._options.numberOfShards);
        const partition = this.store.getPartitionId(query.aggregateId, aggregateShard);
        let index = 0;
        let allChannels = []
