...
class ClusteredEventStore extends EventstoreWithProjection {
    ...
    startAllProjections(callback) {
        ...
        // Get all Projections
        self.getProjectionsAsync().then((projections) => {
            projections.forEach((projection) => {
                ...
                if (projectionConfig.partitionBy == 'stream' || _.isFunction(projectionConfig.partitionBy)) {
                  for (const eventstore of self._eventstores) {
                      promises.push(eventstore._project(projectionConfig, false));
                  }
                } else {
                    promises.push(super._project(projectionConfig, false));
                }
            });
    ...
    async _doTaskAssignment(projectionGroup, allProjectionTaskIdsInAllShards) {
        const self = this;
        const singleConcurrentTasks = [];
        const multiConcurrentTasks = [];
        ...
        for (let projectionTaskId of allProjectionTaskIdsInAllShards) {
            ...
            if (splitProjectionTaskId.length > 1 && splitProjectionTaskId[1]) {
                multiConcurrentTasks.push(projectionTaskId);
            } else {
                singleConcurrentTasks.push(projectionTaskId);
            }
        }
        ...
        if (self._multiTaskGroup) {
            ...
            // await self._multiTaskGroup.leave();
            await self._multiTaskGroup.addTasks(multiConcurrentTasks);
        } else {
            debug('CLUSTERED _doTaskAssignment: Has no prior task group');
            self._multiTaskGroup = new TaskAssignmentGroup({
                initialTasks: multiConcurrentTasks,
            ...
            self._multiTaskGroup.on('rebalance', async (updatedAssignments, rebalanceId) => {
                ...
                for(const projectionTaskId of updatedAssignments) {
                    ...
                    if (splitProjectionTaskId.length > 1 && splitProjectionTaskId[1]) {
                        ...
                        debug('CLUSTERED FOUND SHARD:', shard, splitProjectionTaskId);
                ...
                if(distributedAssignmentsLength > 0) {
                    for (const eventstore of this._eventstores) {
                        debug('CLUSTERED FOUND ES:', eventstore.shard, eventstore.options.shard);
                    ...
                    debug('CLUSTERED MULTI EMITTING REBALANCE', updatedAssignments);
        ...
        if (self._singleTaskGroup) {
            debug('CLUSTERED _doTaskAssignment: Has a prior task group');
            // await self._singleTaskGroup.leave();
            await self._singleTaskGroup.addTasks(singleConcurrentTasks);
        } else {
            self._singleTaskGroup = new TaskAssignmentGroup({
                initialTasks: singleConcurrentTasks,
                createRedisClient: self._options.redisCreateClient,
            ...
            self._singleTaskGroup.on('rebalance', async (updatedAssignments, rebalanceId) => {
                debug('CLUSTERED SINGLE EMITTING REBALANCE', updatedAssignments);
    ...
    stopAllProjections(callback) {
        ...
        .then(() => {
            if (self._multiTaskGroup) {
                self._multiTaskGroup.leave();
            }
            if (self._singleTaskGroup) {
                self._singleTaskGroup.leave();
            }
            callback();
        }).catch((err) => {
            console.error(err);
    ...
    deleteProjection(projectionId, done) {
        ...
        .then((projectionTasksIds) => {
            // TODO: Remove from appropriate taskgroup by checking config partitionby
            projectionTasksIds.forEach((projectionTaskIds) => {
                if (self._multiTaskGroup) {
                    self._multiTaskGroup.removeTasks(projectionTaskIds);
                }
                if (self._singleTaskGroup) {
                    self._singleTaskGroup.removeTasks(projectionTaskIds);
                }
            });
