...
EventstoreProjectionMysqlStore.prototype.init = async function() {
    ...
    if (!this._isInit) {
        ...
        const createProjectionsTableQuery = `
        ...
        await this._executeSqlQuery(createProjectionsTableQuery);

        const createProjectionTasksTableQuery = `
            CREATE TABLE IF NOT EXISTS \`${projectionTasksTableName}\`
            (
                projection_task_id VARCHAR(10),
                projection_id VARCHAR(100),
                shard VARCHAR(20) NULL,
                ...
                PRIMARY KEY (projection_task_id),
                UNIQUE (projection_id, shard, partition)
            );`;

        await this._executeSqlQuery(createProjectionTasksTableQuery);
 ...
 * @returns {Promise<void>} - returns a Promise of type void
 */
EventstoreProjectionMysqlStore.prototype.createProjectionTask = async function(projectionId, shard = null, partition = null, offset = 0) {
    ...
    debug('createProjectionTask query', query);
    await this._executeSqlQuery(query, [projectionId, shard, partition, offset]);
};
 ...
 * @returns {Promise<void>} - returns a Promise of type void
 */
EventstoreProjectionMysqlStore.prototype.getProjectionTask = async function(projectionId, shard = null, partition = null) {
    const query = `SELECT * FROM ${this.options.projectionTasksTableName}
                  WHERE projection_id = ? AND shard = ? AND partition = ? LIMIT 1;`;

    debug('getProjectionTask query', query);
    const resultRows = await this._executeSqlQuery(query, [projectionId, shard, partition]);
...
EventstoreProjectionMysqlStore.prototype.clearAll = async function() {
    const projectionsQuery = `TRUNCATE TABLE ${this.options.projectionsTableName};`;
    await this._executeSqlQuery(projectionsQuery);

    const projectionTasksQuery = `TRUNCATE TABLE ${this.options.projectionTasksTableName};`;
    await this._executeSqlQuery(projectionTasksQuery);
};
...
EventstoreProjectionMysqlStore.prototype.getProjections = async function(context) {
    ...
    const query = `SELECT * FROM ${this.options.projectionsTableName} ${contextFilter};`;
 ...
 * @param {String} shard the shard to update
 * @param {String} partition the partition to update
 ...
 * @returns {Promise<void>} - returns a Promise of type void
 */
EventstoreProjectionMysqlStore.prototype.setProcessed = async function(projectionId, shard = null, partition = null, processedDate, newOffset, isIdle) {
    let params;
    let query;
    if (newOffset) {
        query = `UPDATE ${this.options.projectionTasksTableName}
                    SET processed_date = ?,
                        offset = ?,
                        is_idle = ?
                    WHERE projection_id = ? AND shard = ? AND partition = ?;`;
        params = [processedDate, newOffset, isIdle == true ? 1 : 0, projectionId, shard, partition];
    } else {
        query = `UPDATE ${this.options.name}
                    SET processed_date = ?,
                        is_idle = ?
                        WHERE projection_id = ? AND shard = ? AND partition = ?;`;
        params = [processedDate, isIdle == true ? 1 : 0, projectionId, shard, partition];
    }
 ...
 * @returns {Promise<void>} - returns a Promise of type void
 */
EventstoreProjectionMysqlStore.prototype.setState = async function(projectionId, shard = null, partition = null, state) {
    const query = `UPDATE ${this.options.projectionTasksTableName}
                    SET 
                        state = ?
                    WHERE projection_id = ? AND shard = ? AND partition = ?;`;
    const params = [state, projectionId, shard, partition];
 ...
 * @returns {Promise<void>} - returns a Promise of type void
 */
EventstoreProjectionMysqlStore.prototype.setError = async function(projectionId, shard = null, partition = null, error, errorEvent, errorOffset) {
    const query = `UPDATE ${this.options.projectionTasksTableName}
                    SET 
                        error = ?,
                        error_event = ?,
...
EventstoreProjectionMysqlStore.prototype.setOffset = async function(projectionId, shard = null, partition = null, offset) {
    const query = `UPDATE ${this.options.name}
                    SET 
                        offset = ?
                    WHERE projection_id = ? AND shard = ? AND partition = ?;`;
    const params = [offset, projectionId, shard, partition];
...
EventstoreProjectionMysqlStore.prototype.delete = async function(projectionId) {
    const deleteProjectionsTableQuery = `DELETE FROM ${this.options.projectionsTableName}
                    WHERE projection_id = ?;`;
    const params = [projectionId];

    debug('delete query', deleteProjectionsTableQuery, params);
    await this._executeSqlQuery(deleteProjectionsTableQuery, params);

    const deleteProjectionTasksTableQuery = `DELETE FROM ${this.options.projectionTasksTableName}
                    WHERE projection_id = ?;`;
    const params = [projectionId];

    debug('delete query', deleteProjectionTasksTableQuery, params);
    await this._executeSqlQuery(deleteProjectionTasksTableQuery, params);
