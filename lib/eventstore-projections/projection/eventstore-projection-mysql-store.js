 ...
 * @param {String} streamId the streamId to check
 ...
 */
 ...
 EventstoreProjectionMysqlStore.prototype.getProjectionStreamVersion = async function(projectionId, context, aggregate, aggregateId) {
  const query = `SELECT offset FROM ${this.options.name}
    WHERE projection_id = ? AND stream_id = ?;`;
  const params = [projectionId, streamId];

  const result = await this._executeSqlQuery(query, params);
  console.log('PROJECTION STREAM VERSION');
  console.log(result);
