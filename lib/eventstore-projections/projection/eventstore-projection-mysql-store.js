...
EventstoreProjectionMysqlStore.prototype.init = async function () {
    ...
    if (!this._isInit) {
        ...
        (
            projection_id VARCHAR(100) PRIMARY KEY,
            context VARCHAR(50) NULL,
            aggregate VARCHAR(75) NULL,
            aggregate_id VARCHAR(125),
            projection_stream_version BIGINT NULL,
            CONSTRAINT projection_id_stream_id UNIQUE (projection_id, context, aggregate, aggregate_id)
        );`;
...
* @returns {Promise<void>} - returns a Promise of type void
*/
EventstoreProjectionMysqlStore.prototype.setProcessed = async function (projectionTaskId, projectionId, processedDate, newOffset, isIdle, lastEventsPerStream) {
    let updateProjectionTasksParams;
    let updateProjectionTasksQuery;

    let updateProjectionCheckpointParams = [];
    let updateProjectionCheckpointQueries = [];

    if (!_.isEmpty(lastEventsPerStream)) {
        // const eventsByStream = _.groupBy(eventsForCheckpoint, (event) => {
        //     return `${event.context}-${event.aggregate}-${event.aggregateId}`;
        // });

        // _.forOwn(eventsByStream, (eventArray, key) => {
        //     const splitKey = key.split('-');
        //     const context = splitKey[0];
        //     const aggregate = splitKey[1];
        //     const aggregateId = splitKey[2];

        //     updateProjectionCheckpointQueries.push(`UPDATE ${this.options.projectionCheckpointTableName}
        //     SET projection_stream_version = ?
        //     WHERE projection_id = ?
        //     AND context = ?
        //     AND aggregate = ?
        //     AND aggregate_id = ?`);
        //     updateProjectionCheckpointParams.push([])
        // });

        _.forOwn(lastEventsPerStream, (event, key) => {
            const splitKey = key.split('-');
            const context = splitKey[0];
            const aggregate = splitKey[1];
            const aggregateId = splitKey[2];

            updateProjectionCheckpointQueries.push(`INSERT INTO ${this.options.projectionCheckpointTableName} (projection_id, context, aggregate, aggregate_id, projection_stream_version)
            VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE
            projection_stream_version = ?;`);
            updateProjectionCheckpointParams.push([projectionId, context, aggregate, aggregateId, event.streamRevision, event.streamRevision]);
        });
    }

    if (newOffset) {
        updateProjectionTasksQuery = `UPDATE ${this.options.projectionTasksTableName}
            SET processed_date = ?,
            offset = ?,
            is_idle = ?
            WHERE projection_task_id = ?;`;
        updateProjectionTasksParams = [processedDate, newOffset, isIdle == true ? 1 : 0, projectionTaskId];
    } else {
        updateProjectionTasksQuery = `UPDATE ${this.options.projectionTasksTableName}
            SET processed_date = ?,
            is_idle = ?
            WHERE projection_task_id = ?;`;
        updateProjectionTasksParams = [processedDate, isIdle == true ? 1 : 0, projectionTaskId];
    }
    
    debug('setProcessed query', updateProjectionTasksQuery, updateProjectionTasksParams);
    await this._executeSqlQuery(updateProjectionTasksQuery, updateProjectionTasksParams);
    if (!_.isEmpty(updateProjectionCheckpointQueries)) {
        for (let i = 0; i < updateProjectionCheckpointQueries.length; i++) {
            await this._executeSqlQuery(updateProjectionCheckpointQueries[i], updateProjectionCheckpointParams[i]);
        }
    }
};
...
/**
 * @param {String} projectionId the projectionId to check
 * @param {String} context the streamId to check
 * @param {String} aggregate the streamId to check
 * @param {String} aggregateId the streamId to check
 * @returns {Promise<void>}
 */

 EventstoreProjectionMysqlStore.prototype.getProjectionStreamVersion = async function(projectionId, context, aggregate, aggregateId) {
  const query = `SELECT projection_stream_version FROM ${this.options.projectionCheckpointTableName}
    WHERE projection_id = ? AND context = ? AND aggregate = ? AND aggregate_id = ?;`;
  const params = [projectionId, context, aggregate, aggregateId];
...
EventstoreProjectionMysqlStore.prototype.updateProjectionStreamVersion = async function(projectionId, context, aggregate, aggregateId, version) {
  const query = `UPDATE ${this.options.projectionCheckpointTableName}
    ...
    AND aggregate = ?
    AND aggregate_id = ?`;
  const params = [version, projectionId, context, aggregate, aggregateId];
  await this._executeSqlQuery(query, params);
}
