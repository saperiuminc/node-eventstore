...
EventstoreStateList.prototype.delete = async function(listName, index) {
    try {
        await this._insertRow(listName, 'DELETE', index, null);
    } catch (error) {
 ...
 * @returns {void} - returns void. use the cb to get results
 */
EventstoreStateList.prototype.set = async function(listName, index, state) {
    try {
        return await this._insertRow(listName, 'UPDATE', index, state);
    } catch (error) {
 ...
 * @returns {void} - returns void. use the cb to get results
 */
EventstoreStateList.prototype.push = async function(listName, state) {
    // push on end
    try {
        const nextRowIndex = await this._nextRowIndex(listName);
        return await this._insertRow(listName, 'CREATE', nextRowIndex, state);
    } catch (error) {
 ...
 * @returns {void} - returns void. use the cb to get results
 */
EventstoreStateList.prototype._insertRow = async function(listName, type, index, state) {
    try {
        var rowData = {
            row_type: type,
            row_index: index,
