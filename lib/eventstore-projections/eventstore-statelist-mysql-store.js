...
EventstoreStateList.prototype.delete = async function(listName, index) {
    try {
        // get first the latest state and put it in the delete row
        const lastState = await this._get(listName, null, index);

        return await this._insertRow(listName, 
            'DELETE', 
            index, 
            lastState ? lastState.value : null, 
            lastState ? lastState.meta : null);
