...
EventstoreStateList.prototype.delete = async function(listName, index) {
    try {
        ...
        const results = await this._executeSqlQuery(sql);
 ...
 * @returns {Promise}
 */
 EventstoreStateList.prototype.truncate = async function(listName) {
    const resultRaw = await this._pool.raw(`TRUNCATE TABLE ${listName};`);
    return resultRaw[0];
};
 ...
 * @returns {Promise}
 */
 EventstoreStateList.prototype.destroy = async function(listName) {
    const resultRaw = await this._pool.raw(`DROP TABLE IF EXISTS ${listName};`);
    return resultRaw[0];
};
 ...
 */
EventstoreStateList.prototype.deleteList = async function(listName) {
    const resultRaw = await this._pool.raw(`DROP TABLE ${listName};`);
    return resultRaw[0];
};
 ...
 */
EventstoreStateList.prototype._get = async function(listName, lastId, rowIndex) {
    try {
        let idFilter = '';
        if (lastId) {
