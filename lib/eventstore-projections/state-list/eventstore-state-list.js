...
EventstoreStateList.prototype.delete = function(index, cb) {
    ...
    if (cb) {
        this._delete(index).then((results) => cb(null, results)).catch(cb);
    } else {
...
EventstoreStateList.prototype.set = function(index, state, meta, cb) {
    ...
    if (cb) {
        this._set(index, state).then((results) => cb(null, results)).catch(cb);
    } else {
...
EventstoreStateList.prototype.push = function(state, meta, cb) {
    ...
    if (cb) {
        this._push(state).then((results) => cb(null, results)).catch(cb);
    } else {
...
EventstoreStateList.prototype.find = function(filters, cb) {
    ...
    if (cb) {
        this._find(this._getRevision(), filters).then((results) => cb(null, results)).catch(cb);
    } else {
 ...
 */
 EventstoreStateList.prototype.reset = function(cb) {
    ...
    if (cb) {
        this._reset().then((results) => cb(null, results)).catch(cb);
    } else {
        return this._reset();
    }
...
EventstoreStateList.prototype.filter = function(filters, cb) {
    ...
    if (cb) {
        this._filter(this._getRevision(), filters).then((results) => cb(null, results)).catch(cb);
    } else {
...
EventstoreStateList.prototype._filter = async function(revision, filters) {
    try {
        ...
        const rows = await this._store.filter(this._listName, revision, filters);
        return rows;
    } catch (error) {
