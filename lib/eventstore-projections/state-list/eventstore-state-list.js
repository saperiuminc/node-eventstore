 ...
 * @constructor
 */
function EventstoreStateList(store, listName, state, shard, partition) {
    this._store = store;
    this._state = state;
    ...
    this._partitionBy = {
        shard: shard === '-1' ? 0 : parseInt(shard),
        partition: partition === '__unpartitioned' ? 0 : parseInt(partition)
    };
    // NOTE: StateList can pass shard -1 and '__unpartitioned' partition. Deprecate?
    // Do we not implement partitioning if single-concurrent?
    // QnD: If Single-concurrent, just default to shard0 partition0 for now
}
...
EventstoreStateList.prototype.delete = function(index, cb) {
    return nodeify(this._delete(index), cb);
};
...
EventstoreStateList.prototype._delete = async function(index) {
    try {
        // push to end
        const revision = await this._store.delete(this._listName, index, this._partitionBy);
...
EventstoreStateList.prototype.set = function(index, state, meta, cb) {
    return nodeify(this._set(index, state), cb);
};
...
EventstoreStateList.prototype._set = async function(index, state, meta) {
    try {
        // push on end
        const revision = await this._store.set(this._listName, index, state, meta, this._partitionBy);
        this._setRevision(revision);
    } catch (error) {
        console.error('error in statelist.set', error);
        throw error;
...
EventstoreStateList.prototype.push = function(state, meta, cb) {
    return nodeify(this._push(state), cb);
};
...
EventstoreStateList.prototype._push = async function(state, meta) {
    try {
        // push on end
        const revision = await this._store.push(this._listName, state, meta, this._partitionBy);
        this._setRevision(revision);
    } catch (error) {
...
EventstoreStateList.prototype.find = function(filters, cb) {
    return nodeify(this._find(this._getRevision(), filters), cb);
};
...
EventstoreStateList.prototype._find = async function(revision, filters) {
    try {
        ...
        const row = await this._store.find(this._listName, revision, filters, this._partitionBy);
        return row;
    } catch (error) {
...
/**
 * 
 * @param {EventstoreStateListDoneCallback} cb 
 * @returns 
 */
 EventstoreStateList.prototype.reset = function(cb) {
 ...
 */
EventstoreStateList.prototype._reset = async function() {
    try {
        // push on end
        // row has value, index, meta
        await this._store.truncate(this._listName);
    } catch (error) {
        console.error('error in statelist.find', error);
        throw error;
    }
...
EventstoreStateList.prototype.filter = function(filters, cb) {
    return nodeify(this._filter(this._getRevision(), filters), cb);
};
...
EventstoreStateList.prototype._filter = async function(revision, filters) {
    try {
        ...
        const rows = await this._store.filter(this._listName, revision, filters, this._partitionBy);
        return rows;
    } catch (error) {
