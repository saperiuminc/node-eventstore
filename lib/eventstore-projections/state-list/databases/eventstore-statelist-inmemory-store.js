...
EventstoreStateListInMemoryStore.prototype.find = async function(listName, lastId, filters) {
    ...
    if (list) {
        ...
        for (let i = 0; i < filters.length; i++) {
            ...
            const indexList = Object.keys(listIndexFieldItems);
            if (!foundIndexList.length) {
                foundIndexList = indexList;
            } else {
                // find intersection
                foundIndexList = _.filter(foundIndexList, (f) => {
                    return _.some(indexList, (i) => i === f);
                });
            }
        }
        
        if (foundIndexList.length) {
            const firstIndex = foundIndexList[0];
            firstItem = {
                index: firstIndex,
                meta: this._lists[listName][firstIndex] ? this._lists[listName][firstIndex].meta : undefined,
                value: this._lists[listName][firstIndex] ? this._lists[listName][firstIndex].state : undefined
            }
        }
    }
...
EventstoreStateListInMemoryStore.prototype.filter = async function(listName, lastId, filters) {
    ...
    if (list) {
        ...
        if (foundIndexList.length) {
            _.forOwn(foundIndexList, (value) => {
                const item = {
                    index: value,
                    meta: this._lists[listName][value] ? this._lists[listName][value].meta : undefined,
                    value: this._lists[listName][value] ? this._lists[listName][value].state : undefined
                }
