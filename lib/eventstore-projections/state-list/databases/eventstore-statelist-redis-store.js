...
util.inherits(EventstoreStateListRedisStore, StateListStore);
...
EventstoreStateListRedisStore.prototype.set = async function(listName, index, state, meta) {
    ...
    pipeline.set(`eventstore-state-lists:${listName}:items:${index}`, {
        state: state,
        meta: meta
    });
...
EventstoreStateListRedisStore.prototype.find = async function(listName, lastId, filters) {
    ...
    for (let i = 0; i < filters.length; i++) {
        ...
        for (let j = 0; j < results.length; j++) {
            ...
            firstItem = this._redisClient.get(`eventstore-state-lists:${listName}:items:${itemIndex}`);
            return firstItem;
        }
    }
...
EventstoreStateListRedisStore.prototype.filter = async function(listName, lastId, filters) {
    ...
    for (let i = 0; i < filters.length; i++) {
        ...
        for (let j = 0; j < results.length; j++) {
            const item = results[j];
            const itemIndex = item.split(':')[1];
            const item = this._redisClient.get(`eventstore-state-lists:${listName}:items:${itemIndex}`);
            items.push(item);
        }
    }
