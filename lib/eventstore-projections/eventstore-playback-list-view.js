 ...
 * @typedef {Object} EventstorePlaybackListViewIndexHintOptions
 * @property {String} index The index to use provided it matches the filter critera
 * @property {String[]} filterCriteria The filter critera
 */
 ...
 * @property {EventstorePlaybackListViewIndexHintOptions} indexHints the list of indexes to use if it matches a filter criteria
...
EventstorePlaybackListView.prototype._query = async function (start, limit, filters, sort, previousKey, nextKey) {
  try {
    ...
    if (this.options.alias == undefined) {
      ...
      let indexHintString = this._buildIndexHintString(filters);
      ...
      if (indexHintString != undefined && indexHintString.length > 0) {
        getListQuery = getListQuery.replace(/@@indexHint/g, ` ${indexHintString} `);
      } else {
        getListQuery = getListQuery.replace(/@@indexHint/g, ``);
      }
 ...
 */
 EventstorePlaybackListView.prototype._buildIndexHintString = function (filters) {
  let indexHintString = '';

  if (this.options.indexHints && this.options.indexHints.length > 0) {
    let matchingIndex = null;

    for(let i = 0; i < this.options.indexHints.length; i++) {
      const indexHint = this.options.indexHints[i];
      const indexName = indexHint.index;
      const criterias = indexHint.filterCriteria;
