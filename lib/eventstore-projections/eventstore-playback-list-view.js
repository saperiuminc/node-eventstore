...
EventstorePlaybackListView.prototype._query = async function(start, limit, filters, sort) {
    try {
        ...
        if(this.options.alias == undefined) {
            ...
            let whereString = this._listFiltersToFilterString(filters);
            ...
            if(whereString != undefined && whereString.length > 0) {
                getQuery = getQuery.replace(`@@where`, ` WHERE ${whereString} `)
            }
            if(orderString != undefined && orderString.length > 0) {
                getQuery = getQuery.replace(`@@order`, ` ${orderString} `)
            }
            if(limitString != undefined && limitString.length > 0) {
                getQuery = getQuery.replace(`@@limit`, ` ${limitString} `)
            }

            // const countQuery = `SELECT COUNT(1) as total_count FROM ${this.options.listName} WHERE 1 = 1 ${filterString}`;    
            // const resultCount = await this._executeSqlQuery(countQuery);
            const resultRows = await this._executeSqlQuery(getQuery);
 ...
 */
EventstorePlaybackListView.prototype._listFiltersToFilterString = function(filters) {
    let filterString = '';
  ...
  if (filters && filters.length > 0) {
    filters.forEach(function(filter, index) {
        ...
        switch (filter.operator) {
            case 'is':
                {
                  filterString += ` ${prefix} ${this._replaceFieldWithAlias(filter.field)} = '${filter.value}' ${suffix} `;
                }
                break;
            ...
            case 'any':
                {
                    if (filter.value && Array.isArray(filter.value) && filter.value.length > 0) {
                        ...
                        filterString += ` ${prefix} ${this._replaceFieldWithAlias(filter.field)} IN (${inFilter}) ${suffix}`;
                    }
                }
                break;
            ...
            case 'range':
                {
                    ...
                    if (filter.from) {
                        filterString += ` ${this._replaceFieldWithAlias(filter.field)} >= ${filter.from} `;
                    }
                    ...
                    if (filter.to) {
                        filterString += ` ${this._replaceFieldWithAlias(filter.field)} <= ${filter.to} `;
                    }
            ...
            case 'contains':
            {
                if (filter.value) {
                    ...
                    filterString += ` ${prefix} ${this._replaceFieldWithAlias(filter.field)} LIKE ${caseSensitiveFiller} '%${filter.value}%' ${suffix} `;
                }
                break;
            }
            case 'arrayContains':
                ...
                if (filter.value) {
                  ...
                  if (Array.isArray(filter.value)) {
                    filter.value.forEach((value, index) => {
                      filterString += ` JSON_CONTAINS(${this._replaceFieldWithAlias(filter.field)}, '"${value}"')`;
                      if (index !== filter.value.length - 1) {
                        filterString += ' OR ';
                      }
                    });
                  } else {
                    filterString += ` JSON_CONTAINS(${this._replaceFieldWithAlias(filter.field)}, '"${filter.value}"')`;
                  }
                  filterString += ` ${suffix} `;
                }
                break;
            ...
            case 'exists':
            {
              filterString += ` ${prefix} ${this._replaceFieldWithAlias(filter.field)} IS NOT NULL ${suffix} `;
              break;
            }
            case 'notExists':
            {
              filterString += ` ${prefix} ${this._replaceFieldWithAlias(filter.field)} IS NULL ${suffix} `;
              break;
            }
          }
    });
