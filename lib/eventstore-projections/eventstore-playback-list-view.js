...
EventstorePlaybackListView.prototype._executeSqlQueryStream = function(queryText, queryParams) {
  return this._pool.raw(queryText, queryParams).stream();
};
  ...
  try {
    ...
    if (this.options.alias == undefined) {
      ...
      queryTransform._transform = function(rowDataPacket, encoding, done) {
          ...
          data: (rowDataPacket.row_json ? JSON.parse(rowDataPacket.row_json) : undefined),
          meta: (rowDataPacket.meta_json ? JSON.parse(rowDataPacket.meta_json) : undefined)
        });
      }
