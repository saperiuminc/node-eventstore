  ...
  try {
    ...
    if (this.options.alias == undefined) {
      ...
      if (whereString != undefined && whereString.length > 0) {
        ...
        getQuery = getQuery.replace(/@@where/g, ``)
      }
  ...
  if (filters && filters.length > 0) {
    filters.forEach(function (filter, index) {
      ...
      switch (filter.operator) {
        case 'is': {
          filterString += ` ${prefix} ${field} = '${filter.value}' ${suffix} `;
        }
        break;
      ...
      case 'contains': {
        if (filter.value) {
          ...
          filterString += ` ${prefix} ${field} LIKE ${caseSensitiveFiller} '%${filter.value}%' ${suffix} `;
        }
        break;
      }
      case 'arrayContains': {
        if (filter.value) {
          ...
          if (Array.isArray(filter.value)) {
            ...
            filterString += ` JSON_CONTAINS(${field}, '"${filter.value}"')`;
          }
          filterString += ` ${suffix} `;
        }
        break;
