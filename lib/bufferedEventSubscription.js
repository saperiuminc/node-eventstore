...
function BufferedEventSubscription (options) {
    ...
    this.waitableFactory = options.waitableFactory;
...
BufferedEventSubscription.prototype = {
    ...
    _startPolling: async function(min, max) {
        ...
        try {
            this.waitableConsumer = await this.waitableFactory.createWaitableConsumer([self.channel]);
            ...
            await this.waitableConsumer.stopWaitingForSignal();
        } catch (error) {
    ...
    _pollForNotify: async function(minRev) {
        ...
        try {
            const res = await self.waitableConsumer.waitForSignal(self.pollingTimeout);
