...
BufferedEventSubscription.prototype = {
    ...
    deactivate: function() {
        ...
        // self.waitableConsumer.stopWaitingForSignal();
    },
    ...
    _pollForNotify: async function(minRev) {
        ...
        try {
          const waitable = await self.distributedSignal.subscribeWithWaitable(self.channel);
          const signalReceivedTask = new Promise((resolve, reject) => {
              try {
          ...
          const sleepTask = self._sleep(self.pollingTimeout);
          await Promise.race([signalReceivedTask, sleepTask]);
          self.distributedSignal.unsubscribe(waitable.id);  

            // self.waitableConsumer.waitForSignal(self.pollingTimeout);
        } catch (error) {
