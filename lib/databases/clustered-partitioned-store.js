...
// const StreamBufferedStore = require('../stream-buffered/stream-buffered-store');

function ClusteredPartitionedStore(options) {
    ...
    PartitionedStore.call(this, options);
}
    ...
    _connect: async function() {
        try {
            ...
            for (const storeConfig of this._options.clusters) {
                ...
                // TODO: Feb24: Get Specific Store?
                const mysqlStore = new MysqlPartitionedStore(config);

                // if (enableStreamBuffer) {
                //     const streamBufferedStore = new StreamBufferedStore(options, mysqlStore)
    ...
    */
    ...
    _getEvents: async function(query, skips, limit) {
        ...
        if (skips) {
            ...
            offsets = self._partitionedStores[shard].getStartingOffset();
        } else if (!skips && (_.isNil(shard) || shard < 0)) {
            offsets = [];
            self._partitionedStores.forEach((store) => {
                offsets.push(store.getStartingOffset());
            })
        }
    ...
    _getLatestOffset: async function(shard, partition) {
        ...
        if (!_.isNil(shard) && !_.isNil(partition)) {
            return helpers.serializeProjectionOffset(await self._partitionedStores[shard].getLatestOffsetAsync(shard, partition));
        } else {
            const latestOffsets = [];
            for (const store of self._partitionedStores) {
