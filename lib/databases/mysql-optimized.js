...
_.extend(MySqlEventStore.prototype, {
    _connect: async function() {
        try {
            ...
            let minPool = parseInt(this._options.connectionPoolLimit) != NaN ? parseInt(this._options.connectionPoolLimit) : 1;
            let maxPool = parseInt(this._options.connectionPoolLimit) != NaN ? parseInt(this._options.connectionPoolLimit) : 1;
            ...
            await this.pool.raw(`CREATE TABLE IF NOT EXISTS ${this._options.database}.${this.options.undispatchedEventsTableName} (` +
                `id BIGINT NOT NULL AUTO_INCREMENT, ` +
                `event_id VARCHAR(45) NOT NULL, ` +
                `context VARCHAR(45) NOT NULL, ` +
                `payload JSON NOT NULL, ` +
    ...
    _clear: async function() {
        ...
        return Promise.all([
            this.pool.raw(`TRUNCATE TABLE ${self._options.database}.${self.options.eventsTableName}`),
            this.pool.raw(`TRUNCATE TABLE ${self._options.database}.${self.options.snapshotsTableName}`),
            this.pool.raw(`TRUNCATE TABLE ${self._options.database}.${self.options.undispatchedEventsTableName}`)
        ]);
    },

    clear: function(done) {
        this._clear().then(data => done(null, data)).catch(callback);
    },
    ...
    _addEvents: async function(events) {
        ...
        try {
            const sql = ` INSERT INTO  ${self._options.database}.${self.options.eventsTableName} (event_id, context, payload, commit_id, position, stream_id, aggregate, aggregate_id, commit_stamp, commit_sequence, stream_revision, rest_in_commit_stream) VALUES ?;`;
