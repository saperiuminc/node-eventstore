...
_.extend(MySqlEventStore.prototype, {
    _connect: async function() {
        try {
            ...
            let createTableQuery = `CREATE TABLE IF NOT EXISTS \`${this._options.eventsTableName}\` (` +
                ...
                `create_stamp DATETIME(6) NOT NULL, ` +
                `stream_revision MEDIUMINT UNSIGNED NOT NULL, ` +
                `partition_id SMALLINT UNSIGNED NOT NULL, ` +
                `PRIMARY KEY (id, context, aggregate, aggregate_id, stream_revision, partition_id), ` +
            ...
            try {
                ...
                await this.pool.query(`
                    CREATE PROCEDURE ${this._options.insertEventsSPName} (
                        ...
                        SET create_stamp = NOW(6);
                        SET stream_revision = event->>'$.streamRevision';
                        SET partition_id = event->>'$.partitionId';
                        
                        INSERT INTO ${this._options.eventsTableName} (event_id, context, payload, aggregate, aggregate_id, commit_stamp, create_stamp, stream_revision, partition_id)
                        VALUES (event_id, context, payload, aggregate, aggregate_id, commit_stamp, create_stamp, stream_revision, partition_id);
    ...
    */
    ...
    _getLatestOffset: async function(shard, partition) {
        ...
        const resultRaw = await self.pool.execute(`SELECT UNIX_TIMESTAMP(create_stamp) as create_stamp_unix, stream_revision, event_id FROM ${self._options.database}.${self._options.eventsTableName} ${partitionClause} ORDER BY id DESC LIMIT 1`, []);
        
        const rows = resultRaw[0];
        if (rows.length > 0) {
            const row = rows[0];
            const latestOffset = {
                createStamp: row.create_stamp_unix,
    ...
    _storedEventToEvent: function(storedEvent, query) {
        const logicalEvent = {
            ...
            eventSequence: this.getOffsetManager().serializeOffset({
                createStamp: storedEvent.create_stamp_unix,
    ...
    _getEventsQueryBuilder: function(query, skip, limit) {
        ...
        if (!bookmark || _.isNil(bookmark.createStamp) || _.isNil(bookmark.streamRevision) || _.isNil(bookmark.eventId)) {
        ...
        let queryString = `SELECT *, UNIX_TIMESTAMP(create_stamp) as create_stamp_unix FROM ${self._options.eventsTableName} ${partitionClause} `;
        const params = [];
        let queryArray = [];
        ...
        queryString = appendInnerQueryElseWhere(queryString, params.length, ` AND `, ` WHERE `);
        queryString += `(`;
        queryString += `(UNIX_TIMESTAMP(create_stamp) > ?) OR `;
        params.push(bookmark.createStamp);
        queryString += `(UNIX_TIMESTAMP(create_stamp) = ? AND stream_revision > ?) OR `;
        params.push(bookmark.createStamp);
        params.push(bookmark.streamRevision);
        queryString += `(UNIX_TIMESTAMP(create_stamp) = ? AND stream_revision = ? AND event_id > ?) `;
        params.push(bookmark.createStamp);
        params.push(bookmark.streamRevision);
        params.push(bookmark.eventId);
        queryString += `)`;
        queryString = queryString + ` ORDER BY UNIX_TIMESTAMP(create_stamp) ASC, stream_revision ASC, event_id ASC LIMIT ?`
    ...
    _getEventsSinceQueryBuilder: function(date, skip, limit) {
        const self = this;
        let queryString = `SELECT *, UNIX_TIMESTAMP(create_stamp) as create_stamp_unix FROM ${self._options.database}.${self._options.eventsTableName} WHERE commit_stamp >= ? ORDER BY commit_stamp ASC, stream_revision ASC LIMIT ? OFFSET ?`;
    ...
    _getEventsByRevisionQueryBuilder: function(query, revMin, revMax) {
        const self = this;
        let queryString = `SELECT event_id, payload, stream_revision, commit_stamp, UNIX_TIMESTAMP(create_stamp) as create_stamp_unix FROM ${self._options.database}.${self._options.eventsTableName}`;
        const params = [];
