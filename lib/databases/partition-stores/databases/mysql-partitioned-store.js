...
_.extend(MySqlEventStore.prototype, {
    _connect: async function() {
        try {
            ...
            let createTableQuery = `CREATE TABLE IF NOT EXISTS \`${this._options.eventsTableName}\` (` +
                ...
                `INDEX idx_get_events (context, aggregate, commit_stamp, stream_revision, event_id)` +
            `)` +
    ...
    */
    ...
    _getLatestOffset: async function(shard, partition) {
        ...
        const queryString = `SELECT commit_stamp, stream_revision, event_id FROM ${self._options.database}.${self._options.eventsTableName} ${partitionClause} ORDER BY commit_stamp DESC, stream_revision DESC, event_id DESC LIMIT 1`;
        const params = [];
        const resultRaw = await self.pool.execute(queryString, params);
        ...
        if (rows.length > 0) {
            const row = rows[0];
            const latestOffset = {
                commitStamp1: row.commit_stamp,
                streamRevision1: row.stream_revision,
                eventId1: row.event_id,
                commitStamp2: row.commit_stamp,
                streamRevision2: row.stream_revision,
                eventId2: row.event_id
            };
            return self.getOffsetManager().serializeOffset(latestOffset);
        } else {
            return self.getOffsetManager().getStartingSerializedOffset();
        }
    },
    ...
    _storedEventToEvent: function(storedEvent, query) {
        const logicalEvent = {
            ...
            eventSequence: this.getOffsetManager().serializeOffset({
                commitStamp2: storedEvent.commit_stamp,
                streamRevision2: storedEvent.stream_revision,
                eventId2: storedEvent.event_id
            }),
    ...
    _getEventsQueryBuilder: function(query, skip, limit) {
        /**
         *  skip: {
                c1: new Date(offset.commitStamp).getTime(),
                e1: offset.eventId,
                s1: offset.streamRevision,
                c2: new Date(offset.commitStamp).getTime(),
                e2: offset.eventId,
                s2: offset.streamRevision
            }
         */
        const self = this;

        // TODO: Make bookmark optional
        ...
        */
        ...
        //     throw new Error('Missing or invalid: bookmark' + ' ' + skip);
        // }
        ...
        // APPROACH 1: UNION
        if (bookmark.commitStamp1 === bookmark.commitStamp2 
            && bookmark.streamRevision1 === bookmark.streamRevision2 
            && bookmark.eventId1 === bookmark.eventId2) {
                // query X events after O1 (exclusive)
                queryString += `(`;
                queryString += `(commit_stamp > ?)`;
                params.push(bookmark.commitStamp1);
                // queryString += `(commit_stamp = ? AND stream_revision > ?) OR `;
                // params.push(bookmark.commitStamp1);
                // params.push(bookmark.streamRevision1);
                // queryString += `(commit_stamp = ? AND stream_revision = ? AND event_id > ?) `;
                // params.push(bookmark.commitStamp1);
                // params.push(bookmark.streamRevision1);
                // params.push(bookmark.eventId1);
                queryString += `)`;
                
                queryString += ` ORDER BY commit_stamp ASC, stream_revision ASC, event_id ASC LIMIT ?`;
                params.push((!limit || isNaN(limit) ? 0 : limit));
        } else {
            if (!bookmark.commitStamp1) {
                // query all events until O2 (inclusive) AND query X events after O2 (exclusive)
                const clonedQueryString = _.cloneDeep(queryString);
                const clonedParams = _.cloneDeep(params);

                queryString += `(`;
                queryString += `(commit_stamp <= ?) OR `;
                params.push(bookmark.commitStamp2);
                queryString += `(commit_stamp = ? AND stream_revision <= ?) OR `;
                params.push(bookmark.commitStamp2);
                params.push(bookmark.streamRevision2);
                queryString += `(commit_stamp = ? AND stream_revision = ? AND event_id <= ?) `;
                params.push(bookmark.commitStamp2);
                params.push(bookmark.streamRevision2);
                params.push(bookmark.eventId2);

                queryString += `)`;

                queryString += ' UNION ( ';
                queryString += clonedQueryString;
                params.push(...clonedParams);

                queryString += `( commit_stamp > ? ) ORDER BY commit_stamp ASC, stream_revision ASC, event_id ASC LIMIT ? )`;
                params.push(bookmark.commitStamp2);
                params.push((!limit || isNaN(limit) ? 0 : limit));

                queryString += ` ORDER BY commit_stamp ASC, stream_revision ASC, event_id ASC`;
            } else {
                // query all events between O1 (exclusive) and O2 (inclusive AND query X events after O2 (exclusive)
                const clonedQueryString = _.cloneDeep(queryString);
                const clonedParams = _.cloneDeep(params);

                queryString += `(`;
                queryString += `(commit_stamp > ? AND commit_stamp <= ?) OR `;
                params.push(bookmark.commitStamp1);
                params.push(bookmark.commitStamp2);
                queryString += `(commit_stamp = ? AND stream_revision > ? AND stream_revision <= ?) OR `;
                params.push(bookmark.commitStamp2);
                params.push(bookmark.streamRevision1);
                params.push(bookmark.streamRevision2);
                queryString += `(commit_stamp = ? AND stream_revision = ? AND event_id > ? AND event_id <= ?) `;
                params.push(bookmark.commitStamp2);
                params.push(bookmark.streamRevision2);
                params.push(bookmark.eventId1);
                params.push(bookmark.eventId2);
        ...
        // APPROACH 2: Multiple Get Events
        // if (bookmark.length > 1) { // query range
        //     const offset1 = bookmark[0];
        //     const offset2 = bookmark[1];

        //     queryString += `(`;
        //     queryString += `(commit_stamp > ? AND commit_stamp <= ?) OR `;
        //     params.push(offset1.commitStamp);
        //     params.push(offset2.commitStamp);
        //     queryString += `(commit_stamp = ? AND stream_revision > ? AND stream_revision <= ?) OR `;
        //     params.push(offset2.commitStamp);
        //     params.push(offset1.streamRevision);
        //     params.push(offset2.streamRevision);
        //     queryString += `(commit_stamp = ? AND stream_revision = ? AND event_id > ? AND event_id <= ?) `;
        //     params.push(offset2.commitStamp);
        //     params.push(offset2.streamRevision);
        //     params.push(offset1.eventId);
        //     params.push(offset2.eventId);
        //     queryString += `)`;
