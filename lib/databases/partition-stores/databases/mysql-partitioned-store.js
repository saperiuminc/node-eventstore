...
_.extend(MySqlEventStore.prototype, {
    _connect: async function() {
        try {
            ...
            let createTableQuery = `CREATE TABLE IF NOT EXISTS \`${this._options.eventsTableName}\` (` +
                ...
                `partition_by_id VARCHAR(125) NOT NULL, ` +
            ...
            for (let i = 0; i < this._options.partitions; i++) {
                createTableQuery += `PARTITION ${this._getFormattedPartitionId(i)} VALUES IN(${i})${i !== (this._options.partitions - 1) ? ', ': ''}`;
            }
            ...
            await this.pool.query(`CREATE TABLE IF NOT EXISTS \`${this._options.snapshotsTableName}\` (` +
                ...
                `commit_stamp BIGINT UNSIGNED NOT NULL, ` +
                `PRIMARY KEY (id), ` +
                `INDEX idx_get_snapshot (context, aggregate, aggregate_id, partition_by_id, commit_stamp)` +
            `)`
            );
            ...
            try {
                ...
                await this.pool.query(`
                    CREATE PROCEDURE ${this._options.insertEventsSPName} (
                        ...
                        DECLARE partition_by_id VARCHAR(125);
                        ...
                        SET partition_by_id = event->>'$.partitionById';
                        SET partition_id = event->>'$.partitionId';
                        
                        INSERT INTO ${this._options.eventsTableName} (event_id, context, payload, aggregate, aggregate_id, commit_stamp, stream_revision, partition_by_id, partition_id)
                        VALUES (event_id, context, payload, aggregate, aggregate_id, commit_stamp, stream_revision, partition_by_id, partition_id);
        ...
        if (events && events.length > 0) {
            ...
            try {
                for (let index = 0; index < events.length; index++) {
                    ...
                    const partitionById = event.partitionById;
                    ...
                    newEvents = newEvents.concat({
                        ...
                        partitionId: this.getPartitionId(partitionById)
                    });
                }
    ...
    */
    ...
    getPartitionId: function(partitionById) {
        const partitionId = murmurhash(partitionById, 2) % this._options.partitions;
        return partitionId;
    },
    
    _getFormattedPartitionId: function(partitionId) {
    ...
    _getLatestOffset: async function(shard, partition) {
        ...
        const partitionClause = _.isNil(partition) ? '' : `PARTITION (${self._getFormattedPartitionId(partition)})`;
    ...
    _storedSnapshotToSnapshot: function(storedSnapshot, query) {
        const logicalSnapshot = {
            ...
            partitionById: query && query.partitionById ? query.partitionById : storedSnapshot.partition_by_id
        }
    ...
    _storedEventToEvent: function(storedEvent, query) {
        const logicalEvent = {
            ...
            streamRevision: storedEvent.stream_revision,
            partitionById: storedEvent.partition_by_id
        }
    ...
    _getEventsQueryBuilder: function(query, skip, limit) {
        ...
        if (!Array.isArray(query)) {
            if(query.partition != undefined) {
                partitionClause = `PARTITION (${self._getFormattedPartitionId(query.partition)})`;
            }
        } else {
            if(query[0] != undefined && query[0].partition != undefined) {
                partitionClause = `PARTITION (${self._getFormattedPartitionId(query[0].partition)})`;
            }
        }
    ...
    _getEventsByRevisionQueryBuilder: function(query, revMin, revMax) {
        const self = this;
        let queryString = `SELECT event_id, payload, stream_revision, commit_stamp, partition_by_id FROM ${self._options.database}.${self._options.eventsTableName}`;
        const params = [];
    ...
    _getLastEventQueryBuilder: function(query) {
        ...
        // TODO: Implement Partitioning if partitionById is indicated in query
        // if (query && query.aggregateId) {
        //     if (params.length > 0) {
        //         queryString = queryString + ` AND `
        //     } else {
        //         queryString = queryString + ` WHERE `
        //     }
        //     queryString = queryString + `partition_by_id = ?`
        //     params.push(`${query.partitionById ? query.partitionById : query.aggregateId}`);
        // }
    ...
    _getSnapshotQueryBuilder: function(query, revMax) {
        const self = this;
        return {
            queryString: `SELECT id, data, revision, partition_by_id, commit_stamp FROM ${self._options.database}.${self._options.snapshotsTableName} 
                WHERE context = ? AND aggregate = ? AND aggregate_id = ? ORDER BY commit_stamp DESC LIMIT 1`,
            params: [query.context || null, query.aggregate || null, `${query.aggregateId}`]
        }
    }
});
