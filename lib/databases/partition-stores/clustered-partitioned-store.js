...
_.extend(ClusteredPartitionedStore.prototype, {
    ...
    */
    ...
    _getEvents: async function(query, skips, limit) {
        ...
        let offset;
        if (skips) {
            offset = self.getOffsetManager().deserializeOffset(skips);
        } else if (!skips && !_.isNil(shard) && shard >= 0) {
            offset = self.getOffsetManager().getStartingOffset();
        }
        ...
        if (!_.isNil(shard) && shard >= 0) {
            ...
            const finalResults = events.map((event) => {
                event.eventSequence = self.getOffsetManager().serializeOffset(event.eventSequence);
                return event;
            });
            return finalResults;
        } else {
    ...
    _getLatestOffset: async function(shard, partition) {
        ...
        if (!_.isNil(shard) && !_.isNil(partition)) {
            ...
            for (const store of self._partitionedStores) {
                queryCombiner.addRowResults([self.getOffsetManager().deserializeOffset(await store.getLatestOffsetAsync(null, null))]);
            }
            const results = await queryCombiner.getJoinedQueryRows(null);
            const latestEventOffset = results && results.length ? results[0] : null;
            
            return latestEventOffset ? self.getOffsetManager().serializeOffset(self.getOffsetManager().serializeOffset(latestEventOffset)) : null;
        }
    },
