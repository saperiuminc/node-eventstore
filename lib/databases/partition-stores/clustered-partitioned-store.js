    ...
    _connect: async function() {
        try {
            ...
            for (const storeConfig of this._options.clusters) {
                ...
                const mysqlStore = new MysqlPartitionedStore(config, compositeOffsetManager);

                if (this._options.enableProjectionEventStreamBuffer) {
                    const streamBufferedStore = new StreamBufferedStore({}, compositeOffsetManager, mysqlStore);
    ...
    _getEvents: async function(query, skips, limit) {
        ...
        if (!_.isNil(shard) && shard >= 0) {
            ...
            if (skips) {
                ...
                shardedSerializedOffset = partitionedStoreOfShard.getOffsetManager().getStartingSerializedOffset();
            }
