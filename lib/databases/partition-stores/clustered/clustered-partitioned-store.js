...
function ClusteredPartitionedStore(options, offsetManager) {
    ...
    this._sortColumns = ['commitStamp', 'streamRevision', 'eventId'];
...
_.extend(ClusteredPartitionedStore.prototype, {
    ...
    */
    _addEvents: async function(events) {
        const self = this;
        const partitionById = self._getPartitionById(events && events.length > 0 ? events[0] : {});
        if (partitionById) {
            const shard = helpers.getShard(partitionById, self._partitionedStores.length);
    ...
    _getPartitionById: function(event) {
        let partitionById = null;
        if (event) {
            if (event.partitionById) {
                partitionById = event.partitionById;
            } else {
                partitionById = event.aggregateId;
            }
        }
    ...
    _getEvents: async function(query, skips, limit) {
        ...
        if (!Array.isArray(query) && !_.isNil(query.shard)) {
            ...
            // TODO: Review, possible code smell related to repartitioning
        }
        ...
        if (!_.isNil(shard) && shard >= 0) {
            ...
            for (let i = 0; i < self._partitionedStores.length; i++) {
                const store = self._partitionedStores[i];
                const offset = clusteredOffset[i];
                const events = await store.getEventsAsync(query, offset, limit);
                queryCombiner.addRowResults(events);
            }
    ...
    _getLastEvent: async function(query) {
        ...
        if (!Array.isArray(query) && !_.isNil(query.shard)) {
            ...
            shard = query[0].shard;
        } else if (!Array.isArray(query) && !_.isNil(query.partitionById)) {
            shard = helpers.getShard(query.partitionById, self._partitionedStores.length);
        } else if (!Array.isArray(query) && !_.isNil(query.aggregateId)) {
            shard = helpers.getShard(query.aggregateId, self._partitionedStores.length);
        }
        if (!_.isNil(shard) && shard >= 0) {
