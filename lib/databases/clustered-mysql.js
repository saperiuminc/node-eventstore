...
_.extend(ClusteredMySqlEventStore.prototype, {
    ...
    _getEvents: async function(query, skip, limit) {
        ...
        for (const store of self._stores) {
          readableStreams.push(store.streamEvents(query, skip, limit));
        }
    ...
    _streamEvents: async function(query, skip, limit) {
      const self = this;
      const readableStreams = [];
      for (const store of self._stores) {
        readableStreams.push(store.streamEvents(query, skip, limit));
    ...
    _getLastEvent: async function(query) {
      // TODO: optimization: getLastEvent from only 1 shard. Use murmurhash, use aggregate_id --> assume aggregate_id exists
      const self = this;
      const readableStreams = [];
      for (const store of self._stores) {
    ...
    _getLatestOffset: async function() {
        const self = this;
        // TODO: how to find out shard/eventstore num? Just based on index?
        const latestOffsets = [];
        for (const store of self._stores) {
          latestOffsets.push(await store.getLatestOffsetAsync());
        }
        // TODO: serialize latestOffsets
        const latestOffset = JSON.stringify(latestOffsets);
        return latestOffset;
    },
