...
util.inherits(ClusteredMySqlEventStore, Store);
...
_.extend(ClusteredMySqlEventStore.prototype, {
    ...
    */
    ...
    getPartitionId: function(aggregateId, shard) {
        const partitionId = this._stores[shard || 0].getPartitionId(aggregateId)
        return partitionId;
    },
    ...
    _getEvents: async function(query, skips, limit) {
        ...
        if (skips) {
            ...
            offsets = self._stores[shard].getStartingOffset();
        } else if (!skips && (_.isNil(shard) || shard < 0)) {
            offsets = [];
