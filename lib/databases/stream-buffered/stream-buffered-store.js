...
function StreamBufferedStore(options, partitionedStore) {
    const self = this;

    let defaults = {
        streamBufferMemoryThreshold: 0.8,                       // Defaults to 80%
        streamBufferMemoryTTL: (1000 * 60 * 60 * 24)            // Default is 1 day
    };
    ...
    self._projectionEventStreamBufferLRUCleaner.drain = function() {
        ...
        if (heapPercentage >= self.options.streamBufferMemoryThreshold) {
...
util.inherits(StreamBufferedStore, PartitionedStore);

_.extend(StreamBufferedStore.prototype, {
    ...
    // TODO: Move to OffsetManager
    getStartingOffset(partition) {
        return this._partitionedStore.getStartingOffset(partition);
    },
    ...
    addSnapshot: function(snapshot, callback) {
        this._partitionedStore.setEventToDispatched(snapshot, callback);
    },
    ...
    _createProjectionEventStreamBuffer: function(query, channel) {
        ...
        const options = {
            ...
            ttl: self.options.streamBufferTTL,
...
module.exports = StreamBufferedStore;
