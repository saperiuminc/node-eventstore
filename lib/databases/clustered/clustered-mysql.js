...
_.extend(ClusteredMySqlEventStore.prototype, {
    _deserializeProjectionOffset: function(serializedProjectionOffset) {
        return JSON.parse(Buffer.from(serializedProjectionOffset, 'base64').toString('utf8'));
    },
    _serializeProjectionOffset: function(projectionOffset) {
    ...
    _getEvents: async function(query, skips, limit) {
      ...
      const self = this;
      const offsets = this._deserializeProjectionOffset(_.clone(skips));
      ...
      for (const store of self._stores) {
        const offset = offsets[storeIndex];
      ...
      const finalOffsets = _.clone(offsets);
      const finalResults = results.map((event) => {
        ...
        event.eventSequence = this._serializeProjectionOffset(finalOffsets);
        return event;
      });
      return finalResults;
    },
    ...
    _getLatestOffset: async function() {
        ...
        return this._serializeProjectionOffset(latestOffsets);
    },
