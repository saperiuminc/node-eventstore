...
_.extend(ClusteredMySqlEventStore.prototype, {
    ...
    */
    ...
    _getEvents: async function(query, skips, limit) {
        ...
        if (!skips && !_.isNil(shard) && shard >= 0) {
            offsets = helpers.serializeProjectionOffset({
              commitStamp: 0,
              streamRevision: -1,
              eventId: ''
            })
            ...
            offsets = [];
            self._stores.forEach(() => {
                offsets.push(helpers.serializeProjectionOffset({
                  commitStamp: 0,
                  streamRevision: -1,
                  eventId: ''
                }));
    ...
    _getLatestOffset: async function(shard, partition) {
        ...
        if (!_.isNil(shard) && !_.isNil(partition)) {
            return await self._stores[shard].getLatestOffsetAsync(shard, partition);
        } else {
            const latestOffsets = [];
            for (const store of self._stores) {
