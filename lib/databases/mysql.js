...
_.extend(MySqlEventStore.prototype, {
    _connect: async function() {
        try {
            ...
            await this.pool.query(`CREATE TABLE IF NOT EXISTS \`${this._options.database}\`.\`${this._options.eventsTableName}\` (` +
            ...
            await this.pool.query(`CREATE TABLE IF NOT EXISTS \`${this._options.database}\`.\`${this._options.undispatchedEventsTableName}\` (` +
                    `event_id VARCHAR(40) NOT NULL, ` +
                    `aggregate_id VARCHAR(100) NOT NULL, ` +
                    `aggregate VARCHAR(50) NOT NULL, ` +
                    `context VARCHAR(25) NOT NULL, ` +
            ...
            await this.pool.query(`CREATE TABLE IF NOT EXISTS \`${this._options.database}\`.\`${this._options.snapshotsTableName}\` (` +
            ...
            await this.pool.query(`DROP PROCEDURE IF EXISTS \`${this._options.database}\`.\`${this._options.insertEventsSPName}\`;
                CREATE PROCEDURE \`${this._options.database}\`.\`${this._options.insertEventsSPName}\` (
                    IN p_events JSON
                ) BEGIN
                    ...
                    WHILE indx < JSON_LENGTH(p_events) DO
                        ...
                        INSERT INTO \`${this._options.database}\`.\`${this._options.eventsTableName}\` (event_id, context, payload, aggregate, aggregate_id, commit_stamp, stream_revision)
                        VALUES (event_id, context, payload, aggregate, aggregate_id, commit_stamp, stream_revision);

                        INSERT INTO \`${this._options.database}\`.\`${this._options.undispatchedEventsTableName}\` (event_id, context, payload, aggregate, aggregate_id, commit_stamp, stream_revision)
                        VALUES (event_id, context, payload, aggregate, aggregate_id, commit_stamp, stream_revision);
    ...
    _clear: async function() {
        return Promise.all([
            ...
            this.pool.query(`TRUNCATE TABLE \`${this._options.database}\`.\`${this._options.undispatchedEventsTableName}\``),
    ...
    _getEvents: async function(query, skip, limit) {
        ...
        let innerQueryString = `SELECT id FROM ${self._options.database}.${self._options.eventsTableName}`;
        const params = [];
        let queryArray = [];
        ...
        const queryString = `SELECT e.* FROM ( ${innerQueryString} ) o JOIN ${self._options.database}.${self._options.eventsTableName} e ON o.id = e.id`;
    ...
    _getUndispatchedEvents: async function(query) {
        ...
        // let queryString = `SELECT * FROM ${self._options.database}.${self._options.undispatchedEventsTableName}`;
        // const params = [];
        ...
        //     results.push(event);
        // });
        // Temporarily disabled return of undispatchedEvents
        return [];
    ...
    _getLatestOffset: async function() {
        ...
        const resultRaw = await self.pool.execute(`SELECT id FROM ${self._options.database}.${self._options.eventsTableName} ORDER BY id DESC LIMIT 1`, []);
    ...
    getLatestOffset: function(callback) {
