...
_.extend(MySqlEventStore.prototype, {
    ...
    _getEvents: async function(query, skip, limit) {
        const self = this;
        const queryBuilder = self._getEventsQuery(query, skip, limit);
    ...
    _streamEvents: async function(query, skip, limit) {
      ...
      const stream = await self.pool.query({ sql: queryString, values: params }).stream();
      return stream;
    },
    ...
    streamEvents: function(query, skip, limit) {
      this._streamEvents(query, skip, limit)
    },
    ...
    _getEventsQuery: function(query, skip, limit) {
      const self = this;
