    ...
    connect: function(callback) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self._initQuery(
                ...
                `PRIMARY KEY (id), ` + 
                `INDEX aggregate_id_aggregate_context_revision_commitstamp (aggregate_id, aggregate, context, stream_revision, commit_stamp)` +
                `)`, (err) => {
                    if (err) {
            ...
            self._initQuery(
                ...
                `revision INT(11) GENERATED ALWAYS AS (json_unquote(json_extract(snapshot,'$.revision'))) VIRTUAL, ` +
                `commit_stamp BIGINT(20) GENERATED ALWAYS AS (json_unquote(json_extract(snapshot,'$.commitStamp'))) VIRTUAL, ` +
                `PRIMARY KEY (id), ` + 
                `INDEX aggregate_id_aggregate_context_revision_commitstamp (aggregate_id, aggregate, context, revision, commit_stamp)` +
                `)`, (err) => {
                    if (err) {
                        return Promise.reject(err);
                    } else {
