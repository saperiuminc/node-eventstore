...
_.extend(MySqlEventStore.prototype, {
    connect: function(callback) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self._initQuery(
                ...
                `commit_sequence INT(11) NOT NULL DEFAULT 0, ` +
                `payload JSON NULL, ` +
                `PRIMARY KEY (id), ` + 
                `INDEX aggregate_id_aggregate_context_revision (aggregate_id, aggregate, context, commit_stamp)` +
                `)`, (err) => {
                    if (err) {
    ...
    getEvents: function(query, skip, limit, callback) {
        ...
        const queryFunc = async function(conn) {
            let queryString = `SELECT id, aggregate_id, aggregate, context, revision, commit_id, commit_stamp, commit_sequence, payload FROM ${self._options.database}.${self.options.eventsTableName}`;
            const params = [];
    ...
    getEventsSince: function(date, skip, limit, callback) {
        ...
        const queryFunc = async function(conn) {
            let queryString = `SELECT id, aggregate_id, aggregate, context, revision, commit_id, commit_stamp, commit_sequence, payload FROM ${self._options.database}.${self._options.eventsTableName}  WHERE commit_stamp >= ? ORDER BY commit_stamp ASC LIMIT ? OFFSET ?`;
    ...
    getUndispatchedEvents: function(query, callback) {
        ...
        const queryFunc = async function(conn) {
            let queryString = `SELECT id, aggregate, aggregate_id, context, revision, commit_id, commit_stamp, commit_sequence, payload FROM ${self._options.database}.${self.options.undispatchedEventsTableName}`;
            const params = [];
            ...
            resultsAndFields.results.forEach((storedUndispatchedEvent) => {
                results.push({
                    ...
                    commitSequence: storedUndispatchedEvent.commit_sequence,
    ...
    addSnapshot: function(snapshot, callback) {
        ...
        const queryFunc = async function(conn) {
            const newSnapshot = {
                ...
                commit_stamp: new Date(snapshot.commitStamp).getTime(),
    ...
    getSnapshot: function(query, revMax, callback) {
        ...
        const queryFunc = async function(conn) {
            const resultsAndFields = await self._query(
                conn,
                `SELECT id, aggregate_id, aggregate, context, revision, commit_stamp, data FROM ${self._options.database}.${self.options.snapshotsTableName} WHERE aggregate_id = ? AND aggregate = ? AND context = ? ORDER BY revision DESC LIMIT 1`, [`${query.aggregateId}`, query.aggregate, query.context]
            );
