...
_.extend(MySqlEventStore.prototype, {
        ...
        try {
            ...
            try {
                ...
                CREATE PROCEDURE \`${this._options.database}\`.\`${this._options.insertEventsSPName}\` (
                    ...
                    START TRANSACTION;
                    SELECT MAX(id) FROM \`${this._options.database}\`.\`${this._options.eventsTableName}\` FOR UPDATE;
    ...
    _getEvents: async function(query, skip, limit) {
        ...
        let queryString = `SELECT * FROM ${self._options.database}.${self._options.eventsTableName}`;
        const params = [];
        let queryArray = [];
