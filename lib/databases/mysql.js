...
_.extend(MySqlEventStore.prototype, {
    ...
    streamLastEvent: function(query) {
      const self = this;
      const queryBuilder = self._getLastEventQueryBuilder(query);
      const queryString = queryBuilder.queryString;
      const params = queryBuilder.params;
    ...
    _getSnapshot: async function(query, revMax) {
        const self = this;
        const queryBuilder = self._getSnapshotQueryBuilder(query);
        const queryString = queryBuilder.queryString;
        const params = queryBuilder.params;
    ...
    _getLatestOffset: async function(partition) {
        ...
        const partitionClause = _.isNil(partition) ? '' : `PARTITION (${this._getPartitionKey(partition)})`;
    ...
    if (!Array.isArray(query)) {
        if(query.partition != undefined) {
            partitionClause = `PARTITION (${this._getPartitionKey(query.partition)})`;
        }
    } else {
        if(query[0] != undefined && query[0].partition != undefined) {
            partitionClause = `PARTITION (${this._getPartitionKey(query[0].partition)})`;
        }
    }
    ...
    _getEventsByRevisionQueryBuilder: async function(query, revMin, revMax) {
      const self = this;
    ...
    _getLastEventQueryBuilder: async function(query) {
      const self = this;
      let queryString = `SELECT event_id, payload, stream_revision, commit_stamp FROM ${self._options.database}.${self._options.eventsTableName}`;
      const params = [];
      if (query && query.context) {
    ...
    _getSnapshotQueryBuilder: async function(query, revMax) {
      const self = this;
      return {
