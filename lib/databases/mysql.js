...
_.extend(MySqlEventStore.prototype, {
    _connect: async function() {
        try {
            ...
            // TODO: make partitions configurable if we need to scale more. for now, it is hard coded and 25 is enough
    ...
    _getEvents: async function(query, skip, limit) {
        ...
        if (!Array.isArray(query)) {
            if(query.partition != undefined) {
                partitionClause = `PARTITION (p${query.partition})`;
            }
        } else {
            if(query[0] != undefined && query[0].partition != undefined) {
                partitionClause = `PARTITION (p${query[0].partition})`;
            }
        }
