...
_.extend(MySqlEventStore.prototype, {
            ...
            try {
                ...
                for (let index = 0; index < events.length; index++) {
                    ...
                    newEvents.push([event.id,
                        ...
                        event.streamId || event.aggregateId,
                        event.aggregate,
                        event.aggregateId,
                        event.commitStamp,
    ...
    _getEvents: async function(query, skip, limit) {
        ...
        const resultRaw = await self.pool.raw(queryString, params);
    ...
    _getEventsSince: async function(date, skip, limit) {
        ...
        const resultRaw = await self.pool.raw(
            queryString,
            params
        );
        const rows = resultRaw[0];
    ...
    _getLastEvent: async function(query) {
        try {
            ...
            const resultRaw = await this.pool.raw(queryString, params);
            const rows = resultRaw[0];
