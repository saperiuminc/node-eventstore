...
EventPollingSubscription.prototype = {
        ...
        try {
            while (self.hasActivePoller && self.activeSubscriptions && Object.keys(self.activeSubscriptions).length > 0) {
                ...
                if (self.hasActivePoller && self.activeSubscriptions && Object.keys(self.activeSubscriptions).length > 0) {
                    // await self._checkAndEmitNewEvents();
    ...
    /**
     * Private method that checks the eventstore if there are new events.
     * If there are any, those events are offered to the stream buffer and emitted to all active subscribers.
     */
    _checkAndEmitNewEvents: async function(retryAttempts) {
        const self = this;
        ...
        const onError = function(error) {
            if (retryAttempts < self.errorMaxRetryCount) {
                ...
                return new Promise((resolve) => {
                    debug(`_checkAndEmitNewEvents - onError: retrying after ${retryTime/1000} seconds`, self.query);
                    setTimeout(resolve, retryTime);
                }).then(() => {
                    return self._checkAndEmitNewEvents(retryAttempts + 1);
                });
            } else {
                console.error(`_checkAndEmitNewEvents - onError: exceeded max retries: ${retryAttempts}. Giving up . . .`, self.query);
                console.error(error);
