...
const debug = require('debug')('eventstore:eventPollingSubscription');
...
EventPollingSubscription.prototype = {
    ...
    /**
     * Determines whether or not the EventPollingSubscription is active.
     * @returns {boolean} true if the EventPollingSubscription is active, and false otherwise.
     */
    isActive: function() {
        const self = this;
    ...
    /**
     * Closes the EventPollingSubscription, but only if it is inactive.
     */
    close: function() {
        const self = this;
        ...
        try {
            ...
            while(self.catchingUpSubscriptions && self.catchingUpSubscriptions[subscriptionToken] && !isDoneEmittingOldEvents) {
                ...
                if (stream && stream.events.length > 0) {
                    ...
                    const hasPoller = self._checkHasActivePollerForRange(min, max);
     ...
     */
    _checkHasActivePollerForRange: function(min, max){
        const self = this;
