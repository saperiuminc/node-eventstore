...
ProjectionEventStreamBuffer.prototype = {
    ...
    offerEvents: function (events, offset) {
        const self = this;
        if (events) {
            if (self._streamBuffer.length <= 0 || (offset >= self.getOldestEventSequence() && offset <= self.getLatestEventSequence())) {
            ...
            } else if (offset > self.getLatestEventSequence()) {
    ...
    getEventsInBuffer: function (offset, limit) {
        ...
        if (self._streamBuffer.length === 0 || offsetEventSequence < oldestEventSequence - 1 || offsetEventSequence >= latestEventSequence) {
            ...
            let start = 0;
            while (self._streamBuffer[start].eventSequence < offsetEventSequence && start < self._streamBuffer.length) {
                start++;
            }
            let end = start + limit;
