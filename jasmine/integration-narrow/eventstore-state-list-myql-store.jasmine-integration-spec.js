...
const mysqlServer = (function() {
    ...
    return {
        up: async function() {
            const command = `docker run --name ${dockerContainerName} -e MYSQL_ROOT_PASSWORD=${mysqlOptions.password} -e MYSQL_DATABASE=${mysqlOptions.database} -p ${mysqlOptions.port}:3306 -d mysql:5.7`;
