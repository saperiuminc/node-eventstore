/**
 * RESULTS
 * CONCURRENCY=10 PARALLELISM=1 MAX_TIME=20000 WARMUP_TIME=500 DEBUG=DEBUG ENABLE_STREAM_BUFFER=0 NUM_VEHICLES=100000 node jasmine/benchmark/eventstore-projection-stream-buffering.zbench.js
 * 
 * Test: test projections
                               RUN 1                 
 Elapsed Time:                 20007                 
 Concurrency:                  10                    
 Parallelism:                  1                     
 Total OPS:                    744                   
 Response Time [avg p95 max]:  [1.23 3 10]           
 CPU % [avg p95 max]:          [21.24 24.66 24.66]   
 Memory (MB) [avg p95 max]:    [93.52 101.88 102.24] 
 Blocked (ms) [avg p95 max]:   [0.26 0.48 4.64]      
 Extra Stats [avg p95 max]:                          
 - Mysql Connections:          [5.86 6 6]            
 - Mysql Inserts:              [0.48 0 10]           
 - Mysql Updates:              [696.29 896 897]      
 - Mysql Deletes:              [0 0 0]               
 - Mysql Selects:              [8348.9 10770 10771]  
 - Redis Connected Clients:    [3 3 3]               
 - Redis Blocked Clients:      [0 0 0]               
 - Redis Instantaneous OPS:    [22.48 35 51]   
 * 
 * CONCURRENCY=10 PARALLELISM=1 MAX_TIME=20000 WARMUP_TIME=500 DEBUG=DEBUG ENABLE_STREAM_BUFFER=1 NUM_VEHICLES=100000 node jasmine/benchmark/eventstore-projection-stream-buffering.zbench.js
...
Test: test projections
 ...
 Parallelism:                  1                     
 Total OPS:                    853                   
 Response Time [avg p95 max]:  [1.07 9 40]           
 CPU % [avg p95 max]:          [20.93 24.18 24.18]   
 Memory (MB) [avg p95 max]:    [92.54 99.14 99.31]   
 Blocked (ms) [avg p95 max]:   [0.2 0.46 1.02]       
 Extra Stats [avg p95 max]:                          
 - Mysql Connections:          [5.86 6 6]            
 - Mysql Inserts:              [0.48 0 10]           
 - Mysql Updates:              [806.19 1010 1010]    
 - Mysql Deletes:              [0 0 0]               
 - Mysql Selects:              [8943.67 11209 11247] 
 - Redis Connected Clients:    [3 3 3]               
 - Redis Blocked Clients:      [0 0 0]               
 - Redis Instantaneous OPS:    [42.1 53 85]
 */
...
const numberOfVehicles = process.env.NUM_VEHICLES || 100000;
// stream buffer enabled by default
const enableProjectionEventStreamBuffer = (process.env.ENABLE_STREAM_BUFFER == undefined || parseInt(process.env.ENABLE_STREAM_BUFFER) == 1) ? true : false;
const numberOfProjections = 1;
...
zbench('bench eventstore-projection', (z) => {
    ...
    z.setup(async (done, b) => {
        try {
            ...
            eventstore = require('../../index')({
                ...
                context: 'vehicle',
                projectionGroup: 'vehicleGroup',
                enableProjectionEventStreamBuffer: enableProjectionEventStreamBuffer
            });
    ...
    let isProjectionsInitialized = false;

    let playbackSuccessEventLockAcquired = false;
    z.test('test projections', async (b) => {
        b.hold(async () => {
            ...
            const projectionIndex = await redisFactory.createClient('client').incr('projection-id');

            for (let i = projectionIndex; i < numberOfProjections + projectionIndex; i++) {
                
                const projectionConfig = {
                    projectionId: `vehicle-domain-${i}`,
                    projectionName: `Vehicle Domain ${i} Projection`,
                    playbackInterface: {
                        i: i,
                        $init: function() {
                            return {
                    ...
                    outputState: 'true',
                    stateList: [{
                        name: `vehicle_domain_${i}_state_list`,
                        fields: [{
                            name: 'vehicleId',
                            type: 'string'
                        }],
                ...
                if (!playbackSuccessEventLockAcquired) {
                    playbackSuccessEventLockAcquired = true;
    
                    let lastM;
                    eventstore.on('playbackSuccess', (data) => {
                        if (lastM) {
                            lastM.end();
                        }
                        b.measure((m) => {
                    ...
                    // wait for all concurrencies to call projectAsync
                    await sleep(1000);
